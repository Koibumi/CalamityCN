        ...
        foreach (TooltipLine line in tooltips)
            ...
            ItemHelper.TranslateTooltip(item, tooltips, "BestiaryNotes", delegate (TooltipLine tooltip)
            {
                ...
                if (item.type == ModContent.ItemType<SpectralVeil>())
                {
                    ...
                    line.Text = line.Text.Replace("Press " + hotkeySpecVeil + " to consume 25% of your maximum stealth to perform a mid-range teleport and render you momentarily invulnerable", "按下 " + hotkeySpecVeil + " 消耗25%最大潜行值，向鼠标方向进行中距离传送".zh());
                }

                if (item.type == ModContent.ItemType<SandCloak>())
                {
                    string hotkeySand = CalamityKeybinds.SandCloakHotkey.TooltipHotkeyString();
                    line.Text = line.Text.Replace("Press " + hotkeySand + " to consume 10% of your maximum stealth to create a protective dust veil which provides +6 defense and +2 life regen", "按下 " + hotkeySand + " 消耗10%最大潜行值，召唤一个围绕你的破坏性沙漠龙卷持续15秒，并提供6点防御和+2生命恢复".zh());
                }
                ...
                if (item.type == ModContent.ItemType<CelestialJewel>())
                {
                    ...
                    line.Text = line.Text.Replace("Press " + hotkeyCelestial + " to teleport to a random location while no bosses are alive", "没有Boss存活时按下 " + hotkeyCelestial + " 键进行随机传送".zh());
                }
                ...
                if (item.type == ModContent.ItemType<BlunderBooster>())
                {
                    ...
                    line.Text = line.Text.Replace("Press " + hotkeyBlunder + " to consume 25% of your maximum stealth to perform a swift upwards/diagonal dash which leaves a trail of lightning bolts", "按下 " + hotkeyBlunder + " 键消耗你盗贼潜行值的25%来向左上/右上方冲刺并留下追踪闪电".zh());
                }
                ...
                if (item.type == ModContent.ItemType<AstralArcanum>())
                {
                    ...
                    line.Text = line.Text.Replace("Press " + hotkeyArcanum + " to toggle teleportation UI while no bosses are alive", "没有Boss存活时按下 " + hotkeyArcanum + " 键可以打开传送UI".zh());
                }

                if (item.type == ModContent.ItemType<AsgardianAegis>())
                {
                    string hotkeyAsgard = CalamityKeybinds.AegisHotKey.TooltipHotkeyString();
                    line.Text = line.Text.Replace("Press " + hotkeyAsgard + " to activate buffs to all damage, crit chance, and defense", "按下 " + hotkeyAsgard + " 键获得增加伤害、暴击率和防御的增益".zh());
                }

                if (item.type == ModContent.ItemType<ElysianAegis>())
                {
                    string hotkeyElysian = CalamityKeybinds.AegisHotKey.TooltipHotkeyString();
                    line.Text = line.Text.Replace("Press " + hotkeyElysian + " to activate buffs to all damage, crit chance and defense", "按下 " + hotkeyElysian + " 键获得增加伤害、暴击率和防御的增益".zh());
                }
                ...
                if (item.type == ModContent.ItemType<NormalityRelocator>())
                {
                    ...
                    line.Text = line.Text.Replace("Press " + hotkeyReloc + " to teleport to the position of the mouse", "按下 " + hotkeyReloc + " 传送至鼠标位置".zh());
                }
                ...
                if (item.type == ModContent.ItemType<AngelicAlliance>())
                {
                    ...
                    line.Text = line.Text.Replace("Press " + hotkeyAngelic + " to grace yourself in divinity for 15 seconds", "按下 " + hotkeyAngelic + " 键获得15秒神之祝福".zh());
                }
                ...
                if (item.type == ModContent.ItemType<OmegaBiomeBlade>())
                {
                    ...
                    line.Text = line.Text.Replace("Attacking again immediately thereafter dashes to the wheel, shredding anything in the way", "此时立即进行下一次攻击，你将冲向剑刃，并撕裂途经的一切".zh());
                ...
                {
                    line.Text = line.Text.Replace("Terminus", "终末石".zh());
                }
                ...
                */
                ...
                {
                    line.Text = line.Text.Replace("Can mine Demonite, Crimtane, Meteorite, Sea Prisms and Sea Prism Crystals", "可开采魔矿、猩红矿、陨石、海棱晶和海棱晶碎晶".zh());
                }
                ...
                {
                    line.Text = line.Text.Replace("10% increased weapon-type damage while wearing the Shroomite Armor", "穿着蘑菇矿盔甲时提高10%武器类型伤害".zh());
                    line.Text = line.Text.Replace("The weapon type boosted matches which Shroomite helmet is worn", "加成武器类型取决于戴的蘑菇矿头盔的类型".zh());
                }
