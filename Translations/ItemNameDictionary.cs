using System.Collections.Generic;

namespace CalamityCN
{
    public class ItemNameDict
    {
        public Dictionary<string, string> ItemName { get; } = new Dictionary<string, string>()
        {
            {"Abaddon", "魔鬼之庇护"},
            {"AbyssalAmulet", "深渊护身符"},
            {"AbyssalDivingGear", "深渊潜游装"},
            {"AbyssalDivingSuit", "深渊潜游服"},
            {"AbyssalMirror", "深渊魔镜"},
            {"AeroStone", "天蓝石"},
            {"Affliction", "灾劫之尖啸"},
            {"AlchemicalFlask", "冶金烧瓶"},
            {"AlluringBait", "诱惑鱼饵"},
            {"AmalgamatedBrain", "融合之脑"},
            {"AmbrosialAmpoule", "百草天香瓶"},
            {"AmidiasPendant", "阿米迪亚斯之垂饰"},
            {"AmidiasSpark", "阿米迪亚斯电火花石"},
            {"AncientFossil", "远古化石"},
            {"AnechoicPlating", "吸音金属"},
            {"AngelTreads", "天使之靴"},
            {"AquaticEmblem", "海波纹章"},
            {"ArchaicPowder", "古元粉末"},
            {"AsgardianAegis", "阿斯加德之庇护"},
            {"AsgardsValor", "阿斯加德之英勇"},
            {"AstralArcanum", "星辉秘术"},
            {"BadgeofBravery", "勇气勋章"},
            {"BlazingCore", "渎火核心"},
            {"BloodflareCore", "血炎晶核"},
			{"BloodBath", "血浴"},
            {"BloodPact", "血契"},
            {"BloodstainedGlove", "血染手套"},
            {"BloodyWormScarf", "血蠕虫围巾"},
            {"BloodyWormTooth", "血蠕虫之牙"},
            {"BloomStone", "绽花石"},
            {"CalamityRing", "灾劫虚空"},
            {"CelestialJewel", "天魔星石"},
            {"ChaosAmulet", "探险者护符"},
            {"ChaosStone", "混乱石"},
            {"CoinofDeceit", "欺诈硬币"},
            {"CoreOfTheBloodGod", "血神核心"},
            {"CorruptFlask", "腐化药瓶"},
            {"CounterScarf", "反击围巾"},
            {"CrawCarapace", "蟹爪壳"},
            {"CrimsonFlask", "血腥药瓶"},
            {"CrownJewel", "王冠宝石"},
            {"CryoStone", "冰川石"},
            {"DaedalusEmblem", "死亡射手胸针"},
            {"DarkGodsSheath", "暗神之剑鞘"},
            {"DarkSunRing", "蚀日尊戒"},
            {"DeepDiver", "深潜者"},
            {"DeificAmulet", "神圣护符"},
            {"DepthCharm", "深渊咒符"},
            {"DimensionalSoulArtifact", "维魂神物"},
            {"DraedonsHeart", "嘉登之心"},
            {"DragonScales", "浴火龙鳞"},
            {"DynamoStemCells", "痴愚金龙干细胞"},
            {"EclipseMirror", "日蚀魔镜"},
            {"EldritchSoulArtifact", "惧魂神物"},
            {"ElectriciansGlove", "电击手套"},
            {"ElementalGauntlet", "元素之握"},
            {"ElementalQuiver", "元素箭袋"},
            {"ElysianAegis", "极乐之庇护"},
            {"EnchantedPearl", "附魔珍珠"},
            {"EtherealExtorter", "虚空掠夺者"},
            {"EtherealTalisman", "空灵护符"},
            {"EyeoftheStorm", "风暴之眼"},
            {"FabledTortoiseShell", "舐火甲壳"},
            {"FeatherCrown", "天羽头冠"},
            {"FilthyGlove", "污秽手套"},
            {"FleshTotem", "血肉图腾"},
            {"FrostBarrier", "寒冰屏障"},
            {"FrostFlare", "霜冻之炎"},
            {"FungalCarapace", "真菌壳"},
            {"FungalClump", "真菌块"},
            {"FungalSymbiote", "真菌共生体"},
            {"Gehenna", "焦热地狱"},
            {"GiantPearl", "大珍珠"},
            {"GiantShell", "巨大甲壳"},
            {"GiantTortoiseShell", "巨大龟壳"},
            {"GladiatorsLocket", "角斗士金锁"},
            {"GloveOfPrecision", "精准手套"},
            {"GloveOfRecklessness", "鲁莽手套"},
            {"GodlySoulArtifact", "圣魂神物"},
            {"GrandGelatin", "绚彩凝胶"},
            {"GravistarSabaton", "引力靴"},
            {"HarpyRing", "女妖指环"},
            {"HeartofDarkness", "黑暗之心"},
            {"HeartoftheElements", "元灵之心"},
            {"HideofAstrumDeus", "星神隐壳"},
            {"HoneyDew", "蜂蜜露珠"},
            {"InkBomb", "墨水炸弹"},
            {"IronBoots", "铁靴"},
            {"JellyChargedBattery", "凝胶充能电池"},
            {"Laudanum", "鸦片酊"},
            {"LeviathanAmbergris", "利维坦龙涎香"},
            {"LifeJelly", "生命凝胶"},
            {"LivingDew", "生命露珠"},
            {"LumenousAmulet", "流明护身符"},
            {"LureofEnthrallment", "魅惑珍珠"},
            {"LuxorsGift", "卢克索的礼物"},
            {"MagicLevelMeter", "魔法等级测量器"},
            {"ManaJelly", "魔力凝胶"},
            {"ManaOverloader", "魔能谐振仪"},
            {"MeleeLevelMeter", "近战等级测量器"},
            {"MirageMirror", "幻影魔镜"},
            {"MomentumCapacitor", "动量电容器"},
            {"MoonstoneCrown", "月长石头冠"},
            {"Nanotech", "纳米技术"},
            {"NebulousCore", "星云之核"},
            {"NecklaceofVexation", "恼怒项链"},
            {"OceanCrest", "海波项链"},
            {"OldDie", "赌徒骰子"},
            {"OrnateShield", "宝光盾牌"},
            {"PermafrostsConcoction", "佩码·福洛斯特之秘药"},
            {"PlaguedFuelPack", "瘟疫燃料背包"},
            {"PlagueHive", "瘟疫蜂巢"},
            {"ProfanedSoulArtifact", "渎魂神物"},
            {"RadiantOoze", "光泥"},
            {"RaidersTalisman", "掠夺者护符"},
            {"RampartofDeities", "神之壁垒"},
            {"RangedLevelMeter", "远程等级测量器"},
            {"ReaperToothNecklace", "猎魂鲨齿项链"},
            {"Regenator", "再生冰盾"},
            {"RogueEmblem", "盗贼徽章"},
            {"RogueLevelMeter", "盗贼等级测量器"},
            {"RoseStone", "玫瑰石"},
            {"RottenBrain", "腐烂之脑"},
            {"RuinMedallion", "毁灭勋章"},
            {"SamuraiBadge", "阳炎战旗"},
            {"SandCloak", "沙尘披风"},
            {"ShieldoftheOcean", "海之盾"},
            {"SigilofCalamitas", "灾厄符章"},
            {"SilencingSheath", "寂静刀鞘"},
            {"SirensHeart", "渊洋之心"},
            {"SpectralVeil", "幽灵披风"},
            {"SpiritGenerator", "灵魂浮雕"},
            {"Sponge", "化绵留香石"},
            {"StatisBlessing", "斯塔提斯的祝福"},
            {"StatisCurse", "斯塔提斯的诅咒"},
            {"StatisNinjaBelt", "斯塔提斯忍者腰带"},
			{"StatisVoidSash", "斯塔提斯的虚空饰带"},
            {"StatMeter", "属性统计器"},
            {"StressPills", "压力药丸"},
            {"SummonLevelMeter", "召唤等级测量器"},
            {"SupremeBaitTackleBoxFishingStation", "万圣钓鱼台"},
            {"TheAbsorber", "阴阳吸星石"},
            {"TheAmalgam", "聚合之脑"},
            {"TheBee", "蜜蜂护符"},
            {"TheCamper", "露营者"},
            {"TheCommunity", "归一心元石"},
            {"TheEvolution", "进化者"},
            {"TheFirstShadowflame", "始源暗影炎"},
            {"TheTransformer", "变压器"},
            {"ThiefsDime", "盗贼铸币"},
            {"ToxicHeart", "毒疫之心"},
            {"TrinketofChi", "气功念珠"},
            {"UnstablePrism", "不稳定花岗岩核心"},
            {"VampiricTalisman", "吸血鬼符咒"},
            {"Popo", "魔法围巾帽"},
            {"VeneratedLocket", "敬天神盗盒"},
            {"VitalJelly", "活力凝胶"},
            {"VoidofExtinction", "终结虚空"},
            {"VoltaicJelly", "伏打凝胶"},
            {"WifeinaBottle", "元素瓶"},
            {"WifeinaBottlewithBoobs", "稀有元素瓶"},
            {"AureateWings", "玉金喷射器"},
            {"CelestialTracers", "天神之靴"},
            {"DiscordianWings", "幽渊斗篷"},
            {"DrewsWings", "Drew之翼"},
            {"ElysianTracers", "极乐之靴"},
            {"ElysianWings", "极乐之翼"},
            {"HadarianWings", "哈德安之翼"},
            {"InfinityBoots", "六翼天使之靴"},
            {"MOAB", "气球之母"},
            {"SkylineWings", "天羽之翼"},
            {"SoulofCryogen", "极寒之魂"},
            {"StarlightWings", "星光之翼"},
            {"TarragonWings", "龙蒿叶之翼"},
            {"WulfrumBattery", "钨钢电池"},
            {"YharimsGift", "魔君的礼物"},
            {"YharimsInsignia", "魔君之证章"},
            {"AccelerationBullet", "加速弹"},
            {"AcidBullet", "酸液弹"},
            {"ArcticArrow", "北极箭"},
            {"AstralSolution", "星源溶剂"},
            {"BloodfireArrow", "血炎箭"},
            {"BloodRune", "血之符文"},
            {"ElysianArrow", "极乐之箭"},
            {"EnhancedNanoRound", "注魔纳米弹"},
            {"ExplosiveShells", "爆炸散弹"},
            {"GrenadeRounds", "榴弹片"},
            {"MagnumRounds", "马格南弹"},
            {"FlashBullet", "眩晕弹"},
            {"HolyFireBullet", "圣火弹"},
            {"HyperiusBullet", "海伯里斯弹"},
            {"IcicleArrow", "冰坠箭"},
            {"IcyBullet", "冰砾弹"},
            {"MortarRound", "迫击炮弹"},
            {"NapalmArrow", "汽油箭"},
            {"RubberMortarRound", "橡胶迫击炮"},
            {"SuperballBullet", "弹球弹"},
            {"TerraArrow", "泰拉箭"},
            {"TerraBullet", "泰拉弹"},
            {"VanquisherArrow", "湮灭之箭"},
            {"VeriumBullet", "钫弹"},
            {"AerospecBreastplate", "天蓝胸甲"},
            {"AerospecHat", "天蓝帽"},
            {"AerospecHeadgear", "天蓝头套"},
            {"AerospecHelm", "天蓝战盔"},
            {"AerospecHelmet", "天蓝头盔"},
            {"AerospecHood", "天蓝兜帽"},
            {"AerospecLeggings", "天蓝护胫"},
            {"AstralBreastplate", "星幻胸甲"},
            {"AstralHelm", "星幻头盔"},
            {"AstralLeggings", "星幻护胫"},
            {"AtaxiaArmor", "渊泉盔甲"},
            {"AtaxiaHeadgear", "渊泉头套"},
            {"AtaxiaHelm", "渊泉战盔"},
            {"AtaxiaHelmet", "渊泉头盔"},
            {"AtaxiaHood", "渊泉兜帽"},
            {"AtaxiaMask", "渊泉面具"},
            {"AtaxiaSubligar", "渊泉战裙"},
            {"AuricTeslaBodyArmor", "古圣金源盔甲"},
            {"AuricTeslaCuisses", "古圣金源腿甲"},
            {"AuricTeslaHelm", "金源耀日盔"},
            {"AuricTeslaHoodedFacemask", "金兜铁面盔"},
            {"AuricTeslaPlumedHelm", "金羽十杀盔"},
            {"AuricTeslaSpaceHelmet", "金宇星界盔"},
            {"AuricTeslaWireHemmedVisage", "金虚万象盔"},
            {"BloodflareBodyArmor", "血炎铠甲"},
            {"BloodflareCuisses", "血炎护胫"},
            {"BloodflareHelm", "血饮魔精盔"},
            {"BloodflareHelmet", "血戮狂龙盔"},
            {"BloodflareHornedHelm", "血弑曲角盔"},
            {"BloodflareHornedMask", "血魇九头盔"},
            {"BloodflareMask", "血碾溃阵盔"},
            {"DaedalusBreastplate", "代达罗斯胸甲"},
            {"DaedalusHat", "代达罗斯兜帽"},
            {"DaedalusHeadgear", "代达罗斯头饰"},
            {"DaedalusHelm", "代达罗斯头盔"},
            {"DaedalusHelmet", "代达罗斯头罩"},
            {"DaedalusLeggings", "代达罗斯护胫"},
            {"DaedalusVisor", "代达罗斯面具"},
            {"DemonshadeBreastplate", "魔影装甲"},
            {"DemonshadeGreaves", "魔影护胫"},
            {"DemonshadeHelm", "魔影头盔"},
            {"FearmongerGreathelm", "神惧者头盔"},
            {"FearmongerGreaves", "神惧者护胫"},
            {"FearmongerPlateMail", "神惧者板甲"},
            {"GodSlayerChestplate", "弑神者胸甲"},
            {"GodSlayerHelm", "弑神者头盔"},
            {"GodSlayerHelmet", "弑神者头套"},
            {"GodSlayerHornedHelm", "弑神者角盔"},
            {"GodSlayerLeggings", "弑神者腿甲"},
            {"GodSlayerMask", "弑神者面具"},
            {"GodSlayerVisage", "弑神者面罩"},
            {"MolluskShelleggings", "软壳护胫"},
            {"MolluskShellmet", "软壳贝盔"},
            {"MolluskShellplate", "软壳贝甲"},
            {"OmegaBlueChestplate", "蓝色欧米茄胸甲"},
            {"OmegaBlueHelmet", "蓝色欧米茄头盔"},
            {"OmegaBlueLeggings", "蓝色欧米茄触手"},
            {"PlagueReaperMask", "瘟疫死神面罩"},
            {"PlagueReaperStriders", "瘟疫死神长靴"},
            {"PlagueReaperVest", "瘟疫死神夹克"},
            {"ReaverCuisses", "掠夺者护胫"},
            {"ReaverHelm", "掠夺者战盔"},
            {"ReaverScaleMail", "掠夺者鳞甲"},
            {"ReaverVisage", "掠夺者面罩"},
            {"ShroomiteVisage", "蘑菇面罩"},
            {"SilvaArmor", "始源林海盔甲"},
            {"SilvaHelm", "始源林海战盔"},
            {"SilvaHelmet", "始源林海头盔"},
            {"SilvaHornedHelm", "始源林海角盔"},
            {"SilvaLeggings", "始源林海护胫"},
            {"SilvaMask", "始源林海面具"},
            {"SilvaMaskedCap", "始源林海兜帽"},
            {"StatigelArmor", "斯塔提斯盔甲"},
            {"StatigelCap", "斯塔提斯帽子"},
            {"StatigelGreaves", "斯塔提斯护胫"},
            {"StatigelHeadgear", "斯塔提斯头饰"},
            {"StatigelHelm", "斯塔提斯头盔"},
            {"StatigelHood", "斯塔提斯兜帽"},
            {"StatigelMask", "斯塔提斯面具"},
            {"SulfurBreastplate", "硫磺胸甲"},
            {"SulfurHelmet", "硫磺头盔"},
            {"SulfurLeggings", "硫磺护腿"},
            {"TarragonBreastplate", "龙蒿胸甲"},
            {"TarragonHelm", "龙蒿战盔"},
            {"TarragonHelmet", "龙蒿头盔"},
            {"TarragonHornedHelm", "龙蒿角盔"},
            {"TarragonLeggings", "龙蒿护胫"},
            {"TarragonMask", "龙蒿面具"},
            {"TarragonVisage", "龙蒿兜帽"},
            {"UmbraphileBoots", "日影靴子"},
            {"UmbraphileHood", "日影兜帽"},
            {"UmbraphileRegalia", "日影斗篷"},
            {"AncientGodSlayerChestplate", "远古弑神者胸甲"},
            {"AncientGodSlayerHelm", "远古弑神者头盔"},
            {"AncientGodSlayerLeggings", "远古弑神者护胫"},
            {"ApronOfAffection", "艾斯的慈爱围裙"},
            {"AquaticScourgeMask", "渊海灾虫面具"},
            {"AstrumDeusMask", "星神游龙面具"},
            {"AureusMask", "白金星舰面具"},
            {"BrimstoneWaifuMask", "硫磺火元素面具"},
            {"BumblefuckMask", "痴愚金龙面具"},
            {"CalamitasMask", "灾厄之眼面具"},
            {"CeaselessVoidMask", "无尽虚空面具"},
            {"CrabulonMask", "菌生蟹面具"},
            {"CryogenMask", "极地之灵面具"},
            {"DesertScourgeMask", "荒漠灾虫面具"},
            {"DevourerofGodsMask", "神明吞噬者面具"},
            {"HiveMindMask", "腐巢意志面具"},
            {"LeviathanMask", "利维坦面具"},
            {"PerforatorMask", "血肉宿主面具"},
            {"PlaguebringerGoliathMask", "瘟疫使者歌莉娅面具"},
            {"PolterghastMask", "噬魂幽花面具"},
            {"ProfanedGuardianMask", "亵渎守卫面具"},
            {"ProvidenceMask", "亵渎天神面具"},
            {"RavagerMask", "毁灭魔像面具"},
            {"SignusMask", "西格纳斯面具"},
            {"SlimeGodMask", "史莱姆之神面具"},
            {"SlimeGodMask2", "史莱姆之神面具"},
            {"StormWeaverMask", "风暴编织者面具"},
            {"YharonMask", "犽戎面具"},
            {"VictideBreastplate", "胜潮胸甲"},
            {"VictideHeadgear", "胜潮头蟹"},
            {"VictideHelm", "胜潮战盔"},
            {"VictideHelmet", "胜潮头盔"},
            {"VictideLeggings", "胜潮护胫"},
            {"VictideMask", "胜潮面具"},
            {"VictideVisage", "胜潮头饰"},
            {"WulfrumArmor", "钨钢盔甲"},
            {"WulfrumHeadgear", "钨钢头套"},
            {"WulfrumHelm", "钨钢头盔"},
            {"WulfrumHelmet", "钨钢头罩"},
            {"WulfrumHood", "钨钢面罩"},
            {"WulfrumLeggings", "钨钢护胫"},
            {"WulfrumMask", "钨钢面具"},
            {"XerocCuisses", "皇天护胫"},
            {"XerocMask", "皇天面具"},
            {"XerocPlateMail", "皇天斗篷"},
            {"BabyGhostBellItem", "小鬼铃水母"},
            {"PiggyItem", "小猪猪"},
            {"SeaMinnowItem", "海洋米诺鱼"},
            {"TwinklerItem", "星幻萤火虫"},
            {"Death", "死亡之核"},
            {"Revenge", "复仇之核"},
            {"AldebaranAlewife", "毕宿五灰西鲱"},
            {"ArcturusAstroidean", "大角海星"},
            {"AstralCrate", "星幻匣"},
            {"AstrophageItem", "噬星体"},
            {"GacruxianMollusk", "南十字星蜗"},
            {"PolarisParrotfish", "北辰鹦哥鱼"},
            {"ProcyonidPrawn", "南河三明虾"},
            {"TwinklingPollox", "北河三烁光鱼"},
            {"UrsaSergeant", "大熊座雀鲷"},
            {"Bloodfin", "血鳍"},
            {"Brimlish", "小硫火鱼"},
            {"BrimstoneCrate", "硫磺火匣"},
            {"BrimstoneFish", "硫磺火鱼"},
            {"ChaoticFish", "混乱鱼"},
            {"CharredLasher", "焦黑鞭笞者"},
            {"CoastalDemonfish", "海岸恶魔鱼"},
            {"CragBullhead", "硫火崖回鱼"},
            {"DragoonDrizzlefish", "硫火喷吐鱼"},
            {"Shadowfish", "暗影鱼"},
            {"Slurpfish", "饮食者鱼"},
            {"EnchantedStarfish", "附魔星鱼"},
            {"ChaoticSpreadRod", "岩缝取鱼者"},
            {"EarlyBloomRod", "龙蒿钓竿"},
            {"FeralDoubleRod", "野性双头钓竿"},
            {"HeronRod", "白鹭钓竿"},
            {"NavyFishingRod", "沉沦渊石钓竿"},
            {"SlurperPole", "饮食者钓竿"},
            {"TheDevourerofCods", "鳕鱼吞噬者"},
            {"VerstaltiteFishingRod", "寒元钓竿"},
            {"WulfrumRod", "钨钢钓竿"},
            {"FishofEleum", "冰川鱼"},
            {"FishofFlight", "飞翔之鱼"},
            {"FishofLight", "光明之鱼"},
            {"FishofNight", "暗影之鱼"},
            {"GlimmeringGemfish", "闪光宝石鱼"},
            {"Spadefish", "铲子鱼"},
            {"StuffedFish", "填充草药鱼"},
            {"AbyssalCrate", "深渊匣"},
            {"SunbeamFish", "日光鱼"},
            {"CoralskinFoolfish", "珊瑚拟态鲽"},
            {"EutrophicSandfish", "富养沙地鱼"},
            {"GreenwaveLoach", "绿波泥鳅"},
            {"PrismaticGuppy", "棱晶孔雀鱼"},
            {"RustedJingleBell", "锈蚀铃铛"},
            {"ScarredAngelfish", "负伤天使鱼"},
            {"SerpentsBite", "巨蛇之咬"},
            {"Serpentuna", "巨蛇金枪鱼"},
            {"SparklingEmpress", "闪光皇后鱼"},
            {"SunkenCrate", "沉沦匣"},
            {"SunkenSailfish", "沉沦帆鱼"},
            {"SurfClam", "碎浪蛤"},
            {"Xerocodile", "克希洛克鳄"},
            {"KnowledgeAquaticScourge", "渊海灾虫"},
            {"KnowledgeAstralInfection", "星辉瘟疫"},
            {"KnowledgeAstrumAureus", "白金星舰"},
            {"KnowledgeAstrumDeus", "星神游龙"},
            {"KnowledgeBloodMoon", "血月"},
            {"KnowledgeBrainofCthulhu", "克苏鲁之脑"},
            {"KnowledgeBrimstoneCrag", "硫火之崖"},
            {"KnowledgeBrimstoneElemental", "硫磺火元素"},
            {"KnowledgeBumblebirb", "痴愚金龙"},
            {"KnowledgeCalamitas", "至尊灾厄"},
            {"KnowledgeCalamitasClone", "灾厄之眼"},
            {"KnowledgeCorruption", "腐化之地"},
            {"KnowledgeCrabulon", "菌生蟹"},
            {"KnowledgeCrimson", "猩红之地"},
            {"KnowledgeCryogen", "极地之灵"},
            {"KnowledgeDesertScourge", "荒漠灾虫"},
            {"KnowledgeDestroyer", "钢铁毁灭者"},
            {"KnowledgeDevourerofGods", "神明吞噬者"},
            {"KnowledgeDukeFishron", "猪龙鱼公爵"},
            {"KnowledgeEaterofWorlds", "世界吞噬者"},
            {"KnowledgeEyeofCthulhu", "克苏鲁之眼"},
            {"KnowledgeGolem", "神庙傀儡"},
            {"KnowledgeHiveMind", "腐巢意志"},
            {"KnowledgeKingSlime", "史莱姆王"},
            {"KnowledgeLeviathanandSiren", "利维坦和阿娜希塔"},
            {"KnowledgeLunaticCultist", "拜月教邪教徒"},
            {"KnowledgeMechs", "机械动乱"},
            {"KnowledgeMoonLord", "月球领主"},
            {"KnowledgeOcean", "海洋"},
            {"KnowledgePerforators", "血肉寄生者和宿主"},
            {"KnowledgePlaguebringerGoliath", "瘟疫使者歌莉娅"},
            {"KnowledgePlantera", "世纪之花"},
            {"KnowledgePolterghast", "噬魂幽花"},
            {"KnowledgeProfanedGuardians", "亵渎守卫"},
            {"KnowledgeProvidence", "亵渎天神"},
            {"KnowledgeQueenBee", "蜂后"},
            {"KnowledgeRavager", "毁灭魔像"},
            {"KnowledgeSentinels", "神之仆从"},
            {"KnowledgeSkeletron", "骷髅王"},
            {"KnowledgeSkeletronPrime", "钢铁骷髅王"},
            {"KnowledgeSlimeGod", "史莱姆之神"},
            {"KnowledgeSulphurSea", "硫磺海"},
            {"KnowledgeTwins", "双子魔眼"},
            {"KnowledgeUnderworld", "地狱"},
            {"KnowledgeWallofFlesh", "血肉之墙"},
            {"KnowledgeYharon", "丛林龙，犽戎"},
            {"AerialiteBar", "天蓝锭"},
            {"AncientBoneDust", "上古骨灰"},
            {"ArmoredShell", "装甲外壳"},
            {"AuricBar", "圣金源锭"},
            {"BarofLife", "生命合金"},
            {"BeetleJuice", "甲虫汁"},
            {"BloodOrb", "血珠"},
            {"BloodSample", "血液样本"},
            {"Bloodstone", "血石"},
            {"BloodstoneCore", "血石核心"},
            {"CalamitousEssence", "湮灭余烬"},
            {"CalamityDust", "灾厄尘"},
            {"CoreofCalamity", "灾劫核心"},
            {"CoreofChaos", "混乱核心"},
            {"CoreofCinder", "日光核心"},
            {"CoreofEleum", "冰川核心"},
            {"CosmiliteBar", "宇宙锭"},
            {"CruptixBar", "熔渣锭"},
            {"DarkPlasma", "暗物质"},
            {"DarksunFragment", "日蚀之阴碎片"},
            {"DemonicBoneAsh", "恶魔骨灰"},
            {"DepthCells", "深渊细胞"},
            {"DesertFeather", "荒漠之羽"},
            {"DivineGeode", "神圣晶石"},
            {"DraedonBar", "永恒锭"},
            {"EbonianGel", "枯萎凝胶"},
            {"EffulgentFeather", "闪耀金羽"},
            {"EndothermicEnergy", "恒温能量"},
            {"EssenceofChaos", "混乱精华"},
            {"EssenceofCinder", "日光精华"},
            {"EssenceofEleum", "冰川精华"},
            {"GalacticaSingularity", "星系异石"},
            {"GrandScale", "大狂鲨鳞"},
            {"HellcasterFragment", "龙魂碎片"},
            {"LivingShard", "生命碎片"},
            {"Lumenite", "流明晶"},
            {"MeldBlob", "冥思溶剂"},
            {"MeldiateBar", "冥思构造体"},
            {"MolluskHusk", "软体动物外壳"},
            {"MurkyPaste", "浆糊泥"},
            {"NightmareFuel", "梦魇魔能"},
            {"Phantoplasm", "幻魂"},
            {"PlagueCellCluster", "瘟疫细胞罐"},
            {"PurifiedGel", "纯净凝胶"},
            {"ReaperTooth", "猎魂鲨牙"},
            {"RuinousSoul", "毁灭之灵"},
            {"ShadowspecBar", "魔影锭"},
            {"SolarVeil", "日影面纱"},
            {"Stardust", "星尘"},
            {"StormlionMandible", "风暴之颚"},
            {"TrapperBulb", "猎捕花苞"},
            {"TrueShadowScale", "腐烂物质"},
            {"TwistingNether", "扭曲虚空"},
            {"UeliaceBar", "龙蒿锭"},
            {"UnholyCore", "污痕核心"},
            {"UnholyEssence", "浊火精华"},
            {"VerstaltiteBar", "寒元锭"},
            {"VictideBar", "海洋遗骸"},
            {"VictoryShard", "胜利碎片"},
            {"WulfrumShard", "钨钢金属废料"},
            {"Brimrose", "硫火玫瑰"},
            {"Fabsol", "瓶中公主灵"},
            {"GazeOfCrysthamyr", "克里斯塔米尔的凝视"},
            {"OnyxExcavatorKey", "玛瑙挖掘机钥匙"},
            {"SquishyBeanMount", "可疑的果冻豆"},
            {"TundraLeash", "冻原拴绳"},
            {"NormalityRelocator", "常态定位器"},
            {"BloodOrange", "血橙"},
            {"CometShard", "彗星碎片"},
            {"Dragonfruit", "龙果"},
            {"Ectoheart", "幽魂之心"},
            {"Elderberry", "旧神浆果"},
            {"ElectrolyteGelPack", "电磁凝胶包"},
            {"EtherealCore", "飘渺之核"},
            {"InfernalBlood", "地狱之血"},
            {"MiracleFruit", "奇迹之果"},
            {"MLGRune2", "天体洋葱"},
            {"MushroomPlasmaRoot", "蘑菇根质"},
            {"PhantomHeart", "幻影之心"},
            {"RedLightningContainer", "电刑之容"},
            {"StarlightFuelCell", "星光能量核心"},
            {"BearEye", "猫眼石"},
            {"BloodyVein", "血管碎块"},
            {"BrimstoneJewel", "硫火宝珠"},
            {"CharredRelic", "厄古炭钩"},
            {"CosmicPlushie", "星际毛绒娃娃"},
            {"ForgottenDragonEgg", "被遗忘的龙蛋"},
            {"FoxDrive", "狐盘"},
            {"IbarakiBox", "华扇的百药枡"},
            {"Levi", "利维"},
            {"PrimroseKeepsake", "报春花纪念品"},
            {"RadiatingCrystal", "辐射水晶"},
            {"RomajedaOrchid", "Romajeda之兰"},
            {"RotomRemote", "摩擦发光遥控器"},
            {"StrangeOrb", "奇光球"},
            {"TrashmanTrashcan", "垃圾罐"},
            {"AbyssGravel", "深渊砾石"},
            {"AerialiteBrick", "天蓝砖"},
            {"AstralBar", "炫星锭"},
            {"AstralBrick", "炫星砖"},
            {"AstralClay", "星幻粘土"},
            {"AstralDirt", "星幻土块"},
            {"AstralFossil", "星界残骸"},
            {"AstralIce", "星幻冰雪块"},
            {"AstralMonolith", "星幻木材"},
            {"AstralSand", "星幻沙块"},
            {"AstralSandstone", "星幻沙岩块"},
            {"AstralSilt", "新星熔渣"},
            {"AstralSnow", "星幻雪块"},
            {"AstralStone", "星幻石块"},
            {"BrimstoneSlag", "硫磺火石砖"},
            {"ChaoticBrick", "混乱砖"},
            {"Cinderplate", "狱烬石"},
            {"CryonicBrick", "寒元砖"},
            {"EutrophicSand", "富营养沙"},
            {"AstralChest", "星幻箱"},
            {"BlueCandle", "失重蜡烛"},
            {"ChaosCandle", "混乱蜡烛"},
            {"CorruptionEffigy", "腐化塑像"},
            {"AncientAltar", "远古祭坛"},
            {"AshenAltar", "灰烬祭坛"},
            {"BotanicPlanter", "植物培养器"},
            {"DraedonsForge", "嘉登熔炉"},
            {"EutrophicCrafting", "Eutrophic Crafting"},
            {"MonolithCrafting", "Monolith Crafting"},
            {"PlagueInfuser", "瘟疫灌注器"},
            {"ProfanedBasin", "Profaned Basin"},
            {"SilvaBasin", "Silva Basin"},
            {"StaticRefiner", "静电精炼机"},
            {"VoidCondenser", "虚空压缩机"},
            {"CrimsonEffigy", "血腥塑像"},
            {"PinkCandle", "活力蜡烛"},
            {"PurpleCandle", "恢复蜡烛"},
            {"TranquilityCandle", "宁静蜡烛"},
            {"AquaticScourgeTrophy", "渊海灾虫纪念章"},
            {"AstrageldonTrophy", "白金星舰纪念章"},
            {"AstrumDeusTrophy", "星神游龙纪念章"},
            {"BrimstoneElementalTrophy", "硫磺火元素纪念章"},
            {"BumblebirbTrophy", "痴愚金龙纪念章"},
            {"CalamitasTrophy", "灾厄之眼纪念章"},
            {"CataclysmTrophy", "灾祸纪念章"},
            {"CatastropheTrophy", "灾难纪念章"},
            {"CeaselessVoidTrophy", "无尽虚空纪念章"},
            {"CrabulonTrophy", "菌生蟹纪念章"},
            {"CryogenTrophy", "极地之灵纪念章"},
            {"DesertScourgeTrophy", "荒漠灾虫纪念章"},
            {"DevourerofGodsTrophy", "神明吞噬者纪念章"},
            {"HiveMindTrophy", "腐巢意志纪念章"},
            {"LeviathanTrophy", "利维坦纪念章"},
            {"PerforatorTrophy", "血肉宿主纪念章"},
            {"PlaguebringerGoliathTrophy", "瘟疫使者歌莉娅纪念章"},
            {"PolterghastTrophy", "噬魂幽花纪念章"},
            {"ProvidenceTrophy", "亵渎天神纪念章"},
            {"RavagerTrophy", "毁灭魔像纪念章"},
            {"SignusTrophy", "西格纳斯纪念章"},
            {"SlimeGodTrophy", "史莱姆之神纪念章"},
            {"WeaverTrophy", "风暴编织者纪念章"},
            {"YharonTrophy", "犽戎纪念章"},
            {"YellowCandle", "恶念蜡烛"},
            {"AbyssBed", "深渊床"},
            {"AbyssBookcase", "深渊书架"},
            {"AbyssCandelabra", "深渊烛台"},
            {"AbyssCandle", "深渊蜡烛"},
            {"AbyssChair", "深渊椅"},
            {"AbyssChandelier", "深渊吊灯"},
            {"AbyssChest", "深渊箱"},
            {"AbyssClock", "深渊钟"},
            {"AbyssDoor", "深渊门"},
            {"AbyssDresser", "深渊梳妆台"},
            {"AbyssLamp", "深渊灯"},
            {"AbyssLantern", "深渊灯笼"},
            {"AbyssPiano", "深渊钢琴"},
            {"AbyssSink", "深渊水槽"},
            {"AbyssSofa", "深渊沙发"},
            {"AbyssTable", "深渊桌"},
            {"AbyssTorch", "深渊火把"},
            {"AbyssWorkBench", "深渊工作台"},
            {"SmoothAbyssGravel", "平滑深渊石"},
            {"SmoothAbyssGravelPlatform", "平滑深渊砾石平台"},
            {"AncientBasin", "远古火盆"},
            {"AncientBed", "远古床"},
            {"AncientBookcase", "远古书架"},
            {"AncientChair", "远古椅"},
            {"AncientChandelier", "远古烛台"},
            {"AncientDoor", "远古门"},
            {"AncientDresser", "远古梳妆台"},
            {"AncientLamp", "远古灯"},
            {"AncientLantern", "远古灯笼"},
            {"AncientMonolith", "远古石柱"},
            {"AncientPiano", "远古钢琴"},
            {"AncientSink", "远古水槽"},
            {"AncientSofa", "远古沙发"},
            {"AncientTable", "远古桌"},
            {"AncientWorkBench", "远古工作台"},
            {"AshenBasin", "灰烬火盆"},
            {"AshenBed", "灰烬床"},
            {"AshenBookcase", "灰烬书架"},
            {"AshenCandelabra", "灰烬烛台"},
            {"AshenCandle", "灰烬蜡烛"},
            {"AshenChair", "灰烬椅"},
            {"AshenChandelier", "灰烬吊灯"},
            {"AshenChest", "灰烬箱"},
            {"AshenDoor", "灰烬门"},
            {"AshenDresser", "灰烬梳妆台"},
            {"AshenLamp", "灰烬灯"},
            {"AshenLantern", "灰烬灯笼"},
            {"AshenMonolith", "灰烬柱"},
            {"AshenPiano", "灰烬钢琴"},
            {"AshenPlatform", "灰烬平台"},
            {"AshenSink", "灰烬水槽"},
            {"AshenSlab", "灰烬石板"},
            {"AshenSofa", "灰烬沙发"},
            {"AshenTable", "灰烬桌"},
            {"AshenWorkBench", "灰烬工作台"},
            {"SmoothBrimstoneSlag", "平滑硫火石"},
            {"MonolithBathtub", "星石浴缸"},
            {"MonolithBed", "星石床"},
            {"MonolithBench", "星石长凳"},
            {"MonolithBookcase", "星石书架"},
            {"MonolithCandelabra", "星石烛台"},
            {"MonolithCandle", "星石蜡烛"},
            {"MonolithChair", "星石椅"},
            {"MonolithChandelier", "星石吊灯"},
            {"MonolithChest", "星石箱"},
            {"MonolithClock", "星石钟"},
            {"MonolithDoor", "星石门"},
            {"MonolithDresser", "星石梳妆台"},
            {"MonolithLamp", "星石灯"},
            {"MonolithLantern", "星石灯笼"},
            {"MonolithPiano", "星石钢琴"},
            {"MonolithPlatform", "星石平台"},
            {"MonolithSink", "星石水槽"},
            {"MonolithTable", "星石桌"},
            {"MonolithWorkBench", "星石工作台"},
            {"BotanicBathtub", "植物浴缸"},
            {"BotanicBed", "植物床"},
            {"BotanicBench", "植物长凳"},
            {"BotanicBookcase", "植物书架"},
            {"BotanicCandelabra", "植物烛台"},
            {"BotanicCandle", "植物蜡烛"},
            {"BotanicChair", "植物椅"},
            {"BotanicChandelier", "植物吊灯"},
            {"BotanicChest", "植物箱"},
            {"BotanicClock", "植物钟"},
            {"BotanicDoor", "植物门"},
            {"BotanicDresser", "植物梳妆台"},
            {"BotanicLamp", "植物灯"},
            {"BotanicLantern", "植物灯笼"},
            {"BotanicPiano", "植物钢琴"},
            {"BotanicPlatform", "植物平台"},
            {"BotanicSink", "植物水槽"},
            {"BotanicTable", "植物桌"},
            {"BotanicWorkBench", "植物工作台"},
            {"CosmiliteBasin", "宇宙火盆"},
            {"CosmiliteBed", "宇宙床"},
            {"CosmiliteBookcase", "宇宙书架"},
            {"CosmiliteBrick", "宇宙砖"},
            {"CosmiliteCandelabra", "宇宙烛台"},
            {"CosmiliteCandle", "宇宙蜡烛"},
            {"CosmiliteChair", "宇宙椅"},
            {"CosmiliteChandelier", "宇宙吊灯"},
            {"CosmiliteChest", "宇宙箱"},
            {"CosmiliteClock", "宇宙钟"},
            {"CosmiliteDoor", "宇宙门"},
            {"CosmiliteDresser", "宇宙梳妆台"},
            {"CosmiliteLamp", "宇宙灯"},
            {"CosmiliteLantern", "宇宙灯笼"},
            {"CosmilitePiano", "宇宙钢琴"},
            {"CosmilitePlatform", "宇宙砖平台"},
            {"CosmiliteSconce", "宇宙壁灯"},
            {"CosmiliteSink", "宇宙水槽"},
            {"CosmiliteSofa", "宇宙沙发"},
            {"CosmiliteTable", "宇宙桌"},
            {"CosmiliteWorkBench", "宇宙工作台"},
            {"EutrophicBathtub", "富养浴缸"},
            {"EutrophicBed", "富养床"},
            {"EutrophicBench", "富养长凳"},
            {"EutrophicBookcase", "富养书架"},
            {"EutrophicCandelabra", "富养烛台"},
            {"EutrophicCandle", "富养蜡烛"},
            {"EutrophicChair", "富养椅"},
            {"EutrophicChandelier", "富养吊灯"},
            {"EutrophicChest", "富养箱"},
            {"EutrophicClock", "富养钟"},
            {"EutrophicDoor", "富养门"},
            {"EutrophicDresser", "富养梳妆台"},
            {"EutrophicLamp", "富养灯"},
            {"EutrophicLantern", "富养灯笼"},
            {"EutrophicPiano", "富养钢琴"},
            {"EutrophicPlatform", "富养平台"},
            {"EutrophicSink", "富养水槽"},
            {"EutrophicTable", "富养桌"},
            {"EutrophicWorkBench", "富养工作台"},
            {"SmoothNavystone", "平滑沉沦渊石"},
            {"OccultBathtub", "幻云浴缸"},
            {"OccultBed", "幻云床"},
            {"OccultBookcase", "幻云书架"},
            {"OccultCandelabra", "幻云烛台"},
            {"OccultCandle", "幻云蜡烛"},
            {"OccultChair", "幻云椅"},
            {"OccultChandelier", "幻云烛台"},
            {"OccultChest", "幻云箱"},
            {"OccultClock", "幻云钟"},
            {"OccultDoor", "幻云门"},
            {"OccultDresser", "幻云梳妆台"},
            {"OccultLamp", "幻云灯"},
            {"OccultLantern", "幻云灯笼"},
            {"OccultPiano", "幻云钢琴"},
            {"OccultPlatform", "幻云平台"},
            {"OccultSink", "幻云水槽"},
            {"OccultSofa", "幻云沙发"},
            {"OccultStone", "异域石"},
            {"OccultTable", "幻云桌"},
            {"OccultWorkBench", "幻云工作台"},
            {"PlaguedPlate", "瘟疫密封砖"},
            {"PlaguedPlateBasin", "破损瘟疫‘床’"},
            {"PlaguedPlateBathtub", "瘟疫浴缸"},
            {"PlaguedPlateBed", "瘟疫‘床’"},
            {"PlaguedPlateBookcase", "瘟疫书架"},
            {"PlaguedPlateCandelabra", "瘟疫烛台"},
            {"PlaguedPlateCandle", "瘟疫蜡烛"},
            {"PlaguedPlateChair", "瘟疫椅"},
            {"PlaguedPlateChandelier", "瘟疫吊灯"},
            {"PlaguedPlateChest", "瘟疫箱"},
            {"PlaguedPlateClock", "瘟疫钟"},
            {"PlaguedPlateDoor", "瘟疫门"},
            {"PlaguedPlateDresser", "瘟疫梳妆台"},
            {"PlaguedPlateLamp", "瘟疫灯"},
            {"PlaguedPlateLantern", "瘟疫灯笼"},
            {"PlaguedPlatePiano", "瘟疫钢琴"},
            {"PlaguedPlatePlatform", "瘟疫架"},
            {"PlaguedPlateSink", "瘟疫水槽"},
            {"PlaguedPlateSofa", "瘟疫沙发"},
            {"PlaguedPlateTable", "瘟疫桌"},
            {"PlaguedWorkBench", "瘟疫工作台"},
            {"ProfanedBed", "渎神床"},
            {"ProfanedBench", "渎神长凳"},
            {"ProfanedBookcase", "渎神书架"},
            {"ProfanedCandelabra", "渎神烛台"},
            {"ProfanedCandle", "渎神蜡烛"},
            {"ProfanedChair", "渎神椅"},
            {"ProfanedChandelier", "渎神吊灯"},
            {"ProfanedChest", "渎神箱"},
            {"ProfanedClock", "渎神钟"},
            {"ProfanedCrystal", "渎神结晶"},
            {"ProfanedDoor", "渎神门"},
            {"ProfanedDresser", "渎神梳妆台"},
            {"ProfanedLamp", "渎神灯"},
            {"ProfanedLantern", "渎神灯笼"},
            {"ProfanedPiano", "渎神钢琴"},
            {"ProfanedPlatform", "渎神平台"},
            {"ProfanedRock", "渎神石块"},
            {"ProfanedSink", "渎神水槽"},
            {"ProfanedSlab", "渎神石板"},
            {"ProfanedTable", "渎神桌"},
            {"ProfanedWorkbench", "渎神工作台"},
            {"RunicProfanedBrick", "渎神符文砖"},
            {"SilvaBathtub", "始源林海浴缸"},
            {"SilvaBed", "始源林海床"},
            {"SilvaBench", "始源林海长凳"},
            {"SilvaBookcase", "始源林海书架"},
            {"SilvaCandelabra", "始源林海烛台"},
            {"SilvaCandle", "始源林海蜡烛"},
            {"SilvaChair", "始源林海椅"},
            {"SilvaChandelier", "始源林海吊灯"},
            {"SilvaChest", "始源林海箱"},
            {"SilvaClock", "始源林海钟"},
            {"SilvaCrystal", "始源林海晶块"},
            {"SilvaDoor", "始源林海门"},
            {"SilvaDresser", "始源林海梳妆台"},
            {"SilvaLamp", "始源林海灯"},
            {"SilvaLantern", "始源林海灯笼"},
            {"SilvaPiano", "始源林海钢琴"},
            {"SilvaPlatform", "始源林海平台"},
            {"SilvaSink", "始源林海水槽"},
            {"SilvaTable", "始源林海桌"},
            {"SilvaWorkBench", "始源林海工作台"},
            {"StatigelBed", "斯塔提斯床"},
            {"StatigelBlock", "斯塔提斯块"},
            {"StatigelBookcase", "斯塔提斯书架"},
            {"StatigelCandelabra", "斯塔提斯烛台"},
            {"StatigelCandle", "斯塔提斯蜡烛"},
            {"StatigelChair", "斯塔提斯椅"},
            {"StatigelChandelier", "斯塔提斯吊灯"},
            {"StatigelChest", "斯塔提斯箱"},
            {"StatigelClock", "斯塔提斯钟"},
            {"StatigelDoor", "斯塔提斯门"},
            {"StatigelDresser", "斯塔提斯梳妆台"},
            {"StatigelLamp", "斯塔提斯灯"},
            {"StatigelLantern", "斯塔提斯灯笼"},
            {"StatigelPiano", "斯塔提斯钢琴"},
            {"StatigelPlatform", "斯塔提斯平台"},
            {"StatigelSink", "斯塔提斯水槽"},
            {"StatigelSofa", "斯塔提斯沙发"},
            {"StatigelTable", "斯塔提斯桌"},
            {"StatigelWorkbench", "斯塔提斯工作台"},
            {"StratusBathtub", "幻云浴缸"},
            {"StratusBed", "幻云床"},
            {"StratusBookcase", "幻云书架"},
            {"StratusBricks", "幻云砖块"},
            {"StratusCandelabra", "幻云烛台"},
            {"StratusCandle", "幻云蜡烛"},
            {"StratusChair", "幻云椅"},
            {"StratusChandelier", "幻云吊灯"},
            {"StratusChest", "幻云箱"},
            {"StratusClock", "幻云钟"},
            {"StratusDoor", "幻云门"},
            {"StratusDresser", "幻云梳妆台"},
            {"StratusLamp", "幻云灯"},
            {"StratusLantern", "幻云灯笼"},
            {"StratusPiano", "幻云钢琴"},
            {"StratusPlatform", "幻云平台"},
            {"StratusSink", "幻云水槽"},
            {"StratusSofa", "幻云沙发"},
            {"StratusTable", "幻云桌"},
            {"StratusWorkbench", "幻云工作台"},
            {"SmoothVoidstone", "平滑虚空石块"},
            {"SmoothVoidstonePlatform", "平滑虚空石平台"},
            {"VoidBed", "虚空床"},
            {"VoidBookcase", "虚空书架"},
            {"VoidCandelabra", "虚空烛台"},
            {"VoidCandle", "虚空蜡烛"},
            {"VoidChair", "虚空椅"},
            {"VoidChandelier", "虚空吊灯"},
            {"VoidChest", "虚空箱"},
            {"VoidClock", "虚空钟"},
            {"VoidDoor", "虚空门"},
            {"VoidDresser", "虚空梳妆台"},
            {"VoidLamp", "虚空灯"},
            {"VoidLantern", "虚空灯笼"},
            {"VoidPiano", "虚空钢琴"},
            {"VoidSink", "虚空水槽"},
            {"VoidSofa", "虚空沙发"},
            {"VoidstoneSlab", "虚空石板"},
            {"VoidTable", "虚空桌"},
            {"VoidWorkbench", "虚空工作台"}, 
            {"HardenedAstralSand", "硬化星幻沙块"},
            {"Navystone", "沉沦渊石"},
            {"AerialiteOre", "天蓝矿"},
            {"AstralOre", "炫星矿"},
            {"AuricOre", "圣金源矿"},
            {"ChaoticOre", "熔渣矿"},
            {"CharredOre", "焦炭矿"},
            {"CryonicOre", "寒元矿"},
            {"ExodiumClusterOre", "起源之簇"},
            {"PerennialOre", "永恒矿"},
            {"UelibloomOre", "龙蒿矿"},
            {"PerennialBrick", "永恒砖"},
            {"PlantyMush", "植物混融块"},
            {"PrismShard", "棱晶碎片"},
            {"SeaPrism", "海棱晶"},
            {"SulphurousSand", "硫磺沙"},
            {"SulphurousSandstone", "硫磺沙岩"},
            {"Tenebris", "黑渊石"},
            {"UelibloomBrick", "龙蒿砖"},
            {"Voidstone", "虚空石"},
            {"AbyssGravelWallItem", "深渊砾石墙"},
            {"AerialiteBrickWall", "天蓝砖墙"},
            {"AshenSlabWall", "灰烬石板墙"},
            {"AstralBrickWall", "星幻砖墙"},
            {"AstralDirtWall", "星幻沙墙"},
            {"AstralFossilWall", "星界残骸墙"},
            {"AstralGrassWall", "星幻草墙"},
            {"AstralIceWall", "星幻冰晶墙"},
            {"AstralMonolithWall", "星幻木墙"},
            {"AstralSandstoneWall", "星幻沙岩墙"},
            {"AstralSnowWall", "星幻雪墙"},
            {"AstralStoneWall", "星幻石墙"},
            {"BlueBrickWallUnsafe", "危险蓝砖墙"},
            {"BlueSlabWallUnsafe", "危险蓝板墙"},
            {"BlueTiledWallUnsafe", "危险蓝瓷砖墙"},
            {"BrimstoneSlagWall", "硫火板墙"},
            {"ChaoticBrickWall", "混乱砖墙"},
            {"CinderplateWall", "狱烬石墙"},
            {"CosmiliteBrickWall", "宇宙砖墙"},
            {"CryonicBrickWall", "寒元砖墙"},
            {"EutrophicSandWall", "危险富营养沙墙"},
            {"EutrophicSandWallSafe", "富营养沙墙"},
            {"GreenBrickWallUnsafe", "危险绿砖墙"},
            {"GreenSlabWallUnsafe", "危险绿板墙"},
            {"GreenTiledWallUnsafe", "危险绿瓷砖墙"},
            {"HardenedAstralSandWall", "硬化星幻沙墙"},
            {"NavystoneWall", "危险沉沦渊石墙"},
            {"NavystoneWallSafe", "沉沦渊石墙"},
            {"OccultStoneWall", "幻云石墙"},
            {"PerennialBrickWall", "永恒砖墙"},
            {"PinkBrickWallUnsafe", "危险粉砖墙"},
            {"PinkSlabWallUnsafe", "危险粉板墙"},
            {"PinkTiledWallUnsafe", "危险粉瓷砖墙"},
            {"PlaguedPlateWall", "瘟疫密封墙"},
            {"ProfanedCrystalWall", "渎神结晶墙"},
            {"ProfanedRockWall", "渎神石墙"},
            {"ProfanedSlabWall", "渎神石板墙"},
            {"RunicProfanedBrickWall", "渎神符文砖砖墙"},
            {"SilvaWall", "始源林海墙"},
            {"SmoothAbyssGravelWall", "平滑深渊砾石墙"},
            {"SmoothBrimstoneSlagWall", "平滑硫火石板墙"},
            {"SmoothNavystoneWall", "平滑沉沦渊石墙"},
            {"SmoothVoidstoneWall", "平滑虚空石墙"},
            {"StatigelWall", "斯塔提斯墙"},
            {"StratusWall", "幻云墙"},
            {"SulphurousSandstoneWall", "硫磺沙岩墙"},
            {"SulphurousSandWall", "硫磺沙墙"},
            {"UelibloomBrickWall", "龙蒿砖墙"},
            {"VoidstoneSlabWall", "虚空石板墙"},
            {"VoidstoneWall", "虚空石墙"},
            {"BloodyMary", "血腥玛丽"},
            {"CaribbeanRum", "加勒比朗姆酒"},
            {"CinnamonRoll", "肉桂威士忌"},
            {"Everclear", "谷酒"},
            {"EvergreenGin", "常青松子酒"},
            {"FabsolsVodka", "Fabsol伏特加"},
            {"Fireball", "火龙酒"},
            {"GrapeBeer", "葡萄啤酒"},
            {"Margarita", "玛格丽塔酒"},
            {"Moonshine", "摩闪酒"},
            {"MoscowMule", "莫斯科骡子"},
            {"OddMushroom", "怪异蘑菇"},
            {"RedWine", "红葡萄酒"},
            {"Rum", "朗姆酒"},
            {"Screwdriver", "螺丝起子"},
            {"StarBeamRye", "星流黑麦威士忌"},
            {"Tequila", "龙舌兰酒"},
            {"TequilaSunrise", "龙舌兰日出"},
            {"Vodka", "伏特加"},
            {"Whiskey", "威士忌"},
            {"WhiteWine", "白酒"},
            {"AnechoicCoating", "吸音涂层"},
            {"AstralInjection", "星幻注射剂"},
            {"AstralJelly", "星舰电池"},
            {"Baguette", "法棍"},
            {"BoundingPotion", "弹跳药水"},
            {"CadencePotion", "韵律药水"},
            {"CalamitasBrew", "灾厄之酿"},
            {"CalciumPotion", "钙质药水"},
            {"CeaselessHungerPotion", "无尽吞噬药水"},
            {"CrumblingPotion", "碎甲药水"},
            {"DeliciousMeat", "水晶冻肉"},
            {"DraconicElixir", "龙魂秘药"},
            {"GravityNormalizerPotion", "重力复原药水"},
            {"HolyWrathPotion", "圣神之怒药水"},
            {"OmegaHealingPotion", "终极治疗药水"},
            {"PenumbraPotion", "半影药水"},
            {"PhotosynthesisPotion", "光合药水"},
            {"PotionofOmniscience", "全知药水"},
            {"ProfanedRagePotion", "渎神之怒药水"},
            {"ShatteringPotion", "粉碎药水"},
            {"SoaringPotion", "腾飞药水"},
            {"SunkenStew", "乱渊炖"},
            {"SupremeHealingPotion", "至尊治疗药水"},
            {"SupremeManaPotion", "至尊魔力药水"},
            {"TeslaPotion", "特斯拉药水"},
            {"TitanScalePotion", "泰坦之鳞药水"},
            {"TriumphPotion", "胜利药水"},
            {"YharimsStimulants", "魔君牌兴奋剂"},
            {"ZenPotion", "禅定药水"},
            {"ZergPotion", "虫潮药水"},
            {"Rock", "古恒石"},
            {"Abomination", "瘟疫起动装置"},
            {"AncientMedallion", "唤死笛哨"},
            {"AstralChunk", "星辉碎块"},
            {"BirbPheromones", "奇异信息素"},
            {"BlightedEyeball", "荒芜之眼"},
            {"BloodyWormFood", "血蠕虫诱饵"},
            {"BossRush", "终末石"},
            {"BulbofDoom", "世纪之花花苞"},
            {"CharredIdol", "焦炭玩偶"},
            {"ChickenEgg", "丛林龙蛋"},
            {"CosmicWorm", "宇宙之虫"},
            {"CryoKey", "极寒之匙"},
            {"DecapoditaSprout", "节支动物芽孢"},
            {"DriedSeafood", "荒漠币章"},
            {"EidolonTablet", "幻海灵石"},
            {"EyeofExtinction", "祭魂瓮"},
            {"NecroplasmicBeacon", "死灵魂炬"},
            {"OldPowerCell", "陈旧的能源之核"},
            {"OverloadedSludge", "过载淤泥"},
            {"ProfanedCoreUnlimited", "亵渎晶核"},
            {"ProfanedShard", "亵渎碎片"},
            {"RuneofCos", "宇宙符文"},
            {"SandstormsCore", "沙暴核心"},
            {"Seafood", "海鲜饵料"},
            {"Starcore", "星核"},
            {"Teratoma", "畸形肿瘤"},
            {"SuperDummy", "大草人"},
            {"AbyssalWarhammer", "深渊战锤"},
            {"AerialHamaxe", "天蓝斧锤"},
            {"AstralHamaxe", "幻星锤斧"},
            {"AstralPickaxe", "幻星镐"},
            {"BeastialPickaxe", "野性镐"},
            {"BerserkerWaraxe", "野性斧"},
            {"BlossomPickaxe", "繁花镐"},
            {"BobbitHook", "博比特虫钩"},
            {"ChaoswarpedSlashaxe", "地壳斧正者"},
            {"AridArtifact", "干旱生成器"},
            {"Cosmolight", "宇宙之光"},
            {"TorrentialTear", "洪流之泪"},
            {"CrystylCrusher", "终级碾碎器"},
            {"FellerofEvergreens", "翡翠斧"},
            {"FlamebeakHampick", "震波锤镐"},
            {"GallantPickaxe", "创世镐"},
            {"Gelpick", "凝胶镐"},
            {"Grax", "格拉克斯"},
            {"GreatbayPickaxe", "海湾镐"},
            {"InfernaCutter", "炼狱"},
            {"MarniteObliterator", "花岗石钻头"},
            {"PurityAxe", "纯净之斧"},
            {"ReefclawHamaxe", "暗礁锤斧"},
            {"ShardlightPickaxe", "碎光镐"},
            {"SkyfringePickaxe", "铸天镐"},
            {"WulfrumAxe", "钨钢斧"},
            {"WulfrumHammer", "钨钢锤"},
            {"WulfrumPickaxe", "钨钢镐"},
            {"AquaticScourgeBag", "宝藏袋"},
            {"AstrageldonBag", "宝藏袋"},
            {"AstrumDeusBag", "宝藏袋"},
            {"BrimstoneWaifuBag", "宝藏袋"},
            {"BumblebirbBag", "宝藏袋"},
            {"CalamitasBag", "宝藏袋"},
            {"CrabulonBag", "宝藏袋"},
            {"CryogenBag", "宝藏袋"},
            {"DesertScourgeBag", "宝藏袋"},
            {"DevourerofGodsBag", "宝藏袋"},
            {"HiveMindBag", "宝藏袋"},
            {"LeviathanBag", "宝藏袋"},
            {"PerforatorBag", "宝藏袋"},
            {"PlaguebringerGoliathBag", "宝藏袋"},
            {"PolterghastBag", "宝藏袋"},
            {"ProvidenceBag", "宝藏袋"},
            {"RavagerBag", "宝藏袋"},
            {"SlimeGodBag", "宝藏袋"},
            {"StarterBag", "新手礼包"},
            {"YharonBag", "宝藏袋"},
            {"AbyssalTome", "深渊之书"},
            {"AbyssShocker", "深渊电击者"},
            {"AlphaRay", "阿尔法射线"},
            {"ApoctosisArray", "凋亡射线"},
            {"Apotheosis", "圣神之像"},
            {"AquamarineStaff", "海水杖"},
            {"ArchAmaryllis", "玄法百枝莲"},
            {"ArcticBearPaw", "北极熊爪"},
            {"AsteroidStaff", "行星法杖"},
            {"AstralachneaStaff", "星幻蛛法杖"},
            {"AstralStaff", "幻星法杖"},
            {"Atlantis", "亚特兰蒂斯"},
            {"AuguroftheElements", "先兆元素"},
            {"BlackAnurian", "黑尿鱼"},
            {"BlushieStaff", "蓝链魂火杖"},
            {"BrimroseStaff", "硫火玫瑰"},
            {"BurningSea", "蒸海硫火"},
            {"CalamitasInferno", "混乱火鞭"},
            {"CarnageRay", "屠杀射线"},
            {"ClamorNoctus", "星龙破空杖"},
            {"Climax", "极点光伏"},
            {"ClothiersWrath", "裁缝之怒"},
            {"CoralSpout", "珊瑚泥"},
            {"CosmicRainbow", "宙虹"},
            {"Cryophobia", "恐寒症"},
            {"DarkSpark", "寂虚之光"},
            {"DeathhailStaff", "死亡冰雹"},
            {"DeathValley", "死亡之尘"},
            {"DivineRetribution", "神圣天罚"},
            {"Downpour", "水灌"},
            {"Effervescence", "泡沫冲锋枪"},
            {"EidolicWail", "幻妖龙吟"},
            {"EldritchTome", "诡触之书"},
            {"ElementalRay", "元素射线"},
            {"EventHorizon", "黑洞边缘"},
            {"EvergladeSpray", "常青之气"},
            {"EyeofMagnus", "玛格努斯之眼"},
            {"Fabstaff", "绝学法杖"},
            {"FaceMelter", "变脸熔炉"},
            {"FatesReveal", "命运神启"},
            {"FlareBolt", "火球术"},
            {"ForbiddenSun", "禁忌之阳"},
            {"FrigidflashBolt", "冰火箭魔法书"},
            {"FrostBolt", "冰箭魔法书"},
            {"GammaFusillade", "生化射线"},
            {"Genisis", "源"},
            {"GhastlyVisage", "鬼之形"},
            {"GleamingMagnolia", "烁兰"},
            {"HarvestStaff", "丰收"},
            {"HeliumFlash", "氦闪"},
            {"HellwingStaff", "狱火蝙蝠杖"},
            {"Hematemesis", "血涌"},
            {"HyphaeRod", "菌杖"},
            {"IceBarrage", "寒冰弹幕"},
            {"IcicleStaff", "冰坠法杖"},
            {"IcicleTrident", "冰坠三叉戟"},
            {"InfernalRift", "狱炎裂空"},
            {"IonBlaster", "离子冲击波"},
            {"Judgement", "光之舞"},
            {"Keelhaul", "斥责法杖"},
            {"Lazhar", "拉扎尔射线"},
            {"LightGodsBrilliance", "圣神光辉"},
            {"MadAlchemistsCocktailGlove", "炼金狂人的手套"},
            {"MagnaCannon", "麦格纳炮"},
            {"MagneticMeltdown", "磁场之融"},
            {"ManaRose", "魔法玫瑰"},
            {"Mistlestorm", "槲叶暴风"},
            {"NightsRay", "永夜射线"},
            {"NuclearFury", "原核之怒"},
            {"PhantasmalFury", "幻象之怒"},
            {"PhoenixFlameBarrage", "星火凤凰雨"},
            {"PlagueStaff", "瘟疫法杖"},
            {"PlasmaRifle", "等离子步枪"},
            {"PlasmaRod", "等离子射线"},
            {"Poseidon", "波塞冬"},
            {"PrimordialAncient", "始源之遗"},
            {"PrimordialEarth", "始源之尘"},
            {"Purge", "纳米净化"},
            {"PurgeGuzzler", "净化激光炮"},
            {"RecitationoftheBeast", "狂野复诵"},
            {"RelicofRuin", "遗迹圣物"},
            {"RougeSlash", "殷红鞭笞"},
            {"SanguineFlare", "猩红烈焰"},
            {"SeethingDischarge", "沸腾之火"},
            {"Serpentine", "海蟒咒书"},
            {"ShadecrystalTome", "暗晶风暴"},
            {"ShaderainStaff", "腐蚀之雨法杖"},
            {"ShadowboltStaff", "影流法杖"},
            {"ShiftingSands", "移沙接土"},
            {"SHPC", "SHPC"},
            {"SirensSong", "阿娜希塔琶音"},
            {"SkyGlaze", "天空之羽"},
            {"SnowstormStaff", "冰晶风暴"},
            {"SoulPiercer", "灵魂穿透者"},
            {"StardustStaff", "冰幻法杖"},
            {"Starfall", "流星雨"},
            {"StormSpray", "水流喷射"},
            {"SubsumingVortex", "归元漩涡"},
            {"T1000", "金湮载波炮"},
            {"TearsofHeaven", "天堂之泪"},
            {"TerraRay", "泰拉射线"},
            {"TheSwarmer", "蜂群"},
            {"TheWand", "神杖·暴风灼炎"},
            {"ThornBlossom", "荆棘烁兰"},
            {"Thunderstorm", "雷暴雨"},
            {"TomeofFates", "命运之手"},
            {"Tradewinds", "信风"},
            {"UltraLiquidator", "终结裂空戟"},
            {"UndinesRetribution", "深渊女神之复仇"},
            {"Vehemenc", "狞桀"},
            {"VenusianTrident", "金星三叉戟"},
            {"Vesuvius", "维苏威阿斯"},
            {"Viscera", "心泵血杖"},
            {"VividClarity", "耀界之光"},
            {"VoidVortex", "虚空漩涡"},
            {"Waywasher", "清道夫"},
            {"Wingman", "侧翼"},
            {"WintersFury", "凛冬之怒"},
            {"WrathoftheAncients", "远古之怒"},
            {"WulfrumStaff", "钨钢法杖"},
            {"WyvernsCall", "飞龙之歌"},
            {"YharimsCrystal", "暴君水晶"},
            {"AbsoluteZero", "绝对零度"},
            {"AbyssBlade", "深渊之刃"},
            {"AegisBlade", "庇护之刃"},
            {"Aftershock", "余震"},
            {"AirSpinner", "空旋"},
            {"AmidiasTrident", "海王三叉戟"},
            {"AnarchyBlade", "混乱之刃"},
            {"Aorta", "大动脉"},
            {"AquaticDischarge", "水电剑"},
            {"ArkoftheAncients", "破碎方舟"},
            {"ArkoftheCosmos", "鸿蒙方舟"},
            {"ArkoftheElements", "元素方舟"},
            {"AstralBlade", "幻星刀"},
            {"AstralPike", "幻星矛"},
            {"AstralScythe", "星幻镰刀"},
            {"Ataraxia", "禅心剑"},
            {"Avalanche", "雪崩"},
            {"Azathoth", "阿撒托斯"},
            {"BalefulHarvester", "邪恶收割者"},
            {"BallOFugu", "河豚链球"},
            {"BansheeHook", "鬼爪"},
            {"BiomeBlade", "残缺环境刃"},
            {"BladecrestOathsword", "诅咒之剑"},
            {"BloodyEdge", "血锋"},
            {"Brimlance", "硫磺火矛"},
            {"Brimlash", "硫火鞭"},
            {"BrinyBaron", "海爵剑"},
            {"BurntSienna", "灼炎弯刀"},
            {"Carnage", "屠杀"},
            {"CatastropheClaymore", "灾难巨剑"},
            {"CausticEdge", "绝望之刃"},
            {"CelestialClaymore", "天神之剑"},
            {"Chaotrix", "断层"},
            {"ClamCrusher", "海蚌锤"},
            {"Cnidarian", "珊瑚"},
            {"CometQuasher", "彗星陨刃"},
            {"CosmicDischarge", "寒宙弹刃"},
            {"CosmicShiv", "宇宙短剑"},
            {"CrescentMoon", "新月链刃"},
            {"DarklightGreatsword", "巨剑‘夜光’"},
            {"DepthBlade", "深渊碾碎者"},
            {"Devastation", "破坏重剑"},
            {"DevilsDevastation", "破灭魔王剑"},
            {"DevilsSunrise", "邪锋破晓"},
            {"DiseasedPike", "瘟疫长枪"},
            {"DivineHatchet", "焚罪飞斧"},
            {"DraconicDestruction", "龙破斩"},
            {"DraedonsExoblade", "星流之刃"},
            {"DragonPow", "龙魂破"},
            {"DragonRage", "巨龙之怒"},
            {"Earth", "无限大地"},
            {"EarthenPike", "磐石地元叉"},
            {"ElementalExcalibur", "元素圣剑"},
            {"ElementalShortsword", "元素短剑"},
            {"EmpyreanKnives", "苍穹飞刀"},
            {"EssenceFlayer", "精元劫掠者"},
            {"EutrophicScimitar", "水华弯刀"},
            {"EvilSmasher", "邪恶粉碎者"},
            {"Excelsus", "宙能波纹剑"},
            {"ExsanguinationLance", "火山之矛"},
            {"FeralthornClaymore", "荆棘长刃"},
            {"FlarefrostBlade", "炎霜之刃"},
            {"Floodtide", "鲨潮"},
            {"ForbiddenOathblade", "誓约与禁忌之刃"},
            {"ForsakenSaber", "遗忘之刃"},
            {"FourSeasonsGalaxia", "银河"},
            {"GaelsGreatsword", "盖尔大剑"},
            {"GalacticaBlade", "星河之刃"},
            {"GalaxySmasherMelee", "银河溃灭者"},
            {"GalileoGladius", "伽利略短剑"},
            {"GeliticBlade", "凝胶波刃"},
            {"GildedProboscis", "镀金鸟喙"},
            {"GoldplumeSpear", "金羽"},
            {"GrandGuardian", "大守卫者"},
            {"GreatswordofBlah", "巨剑‘巴拉’"},
            {"GreatswordofJudgement", "巨剑‘审判’"},
            {"Greentide", "翡翠之潮"},
            {"HellfireFlamberge", "狱火重剑"},
            {"HellionFlowerSpear", "狱火花矛"},
            {"Hellkite", "地狱龙锋"},
            {"HolyCollider", "圣火之刃"},
            {"InsidiousImpaler", "阴险穿刺者"},
            {"Lacerator", "裂魔"},
            {"LifefruitScythe", "猎命镰刀"},
            {"Lucrecia", "卢克雷西亚"},
            {"MajesticGuard", "雄伟之护"},
            {"MantisClaws", "螳螂爪"},
            {"MarniteSpear", "花岗石矛"},
            {"MirrorBlade", "镜之刃"},
            {"Mourningstar", "晨星链刃"},
            {"Murasama", "鬼妖村正"},
            {"MycelialClaws", "真菌爪"},
            {"Nadir", "天底"},
            {"Nebulash", "星云鞭"},
            {"NeptunesBounty", "海神之馈赠"},
            {"OldLordOathsword", "旧神之誓约"},
            {"OmegaBiomeBlade", "真·环境之刃"},
            {"Omniblade", "全能之刃"},
            {"Oracle", "神谕"},
            {"PerfectDark", "阴雷"},
            {"PlagueKeeper", "瘟疫大剑"},
            {"ProfanedSword", "硫火之刃"},
            {"PwnagehammerMelee", "圣时之锤"},
            {"Quagmire", "泥潭"},
            {"RedSun", "行星雨"},
            {"RedtideSword", "赤潮"},
            {"Roxcalibur", "石约与胜利之剑"},
            {"RoyalKnivesMelee", "圣光飞刀"},
            {"SausageMaker", "香肠矛"},
            {"ScourgeoftheCosmos", "灾劫宙怨"},
            {"SeashineSword", "海耀之刃"},
            {"Shimmerspark", "烁光"},
            {"SolarFlare", "日炎"},
            {"SolsticeClaymore", "季节长剑"},
            {"SoulEdge", "灵魂之锋"},
            {"SoulHarvester", "灵魂收割者"},
            {"SpatialLance", "元素长枪"},
            {"StarnightLance", "星夜长枪"},
            {"StellarContemptMelee", "星体击碎者"},
            {"StellarStriker", "月炎之锋"},
            {"StormRuler", "风暴管束者"},
            {"StormSaber", "风暴军刀"},
            {"StreamGouge", "宇宙暗流"},
            {"SubmarineShocker", "潜渊震荡者"},
            {"Swordsplosion", "爆破剑"},
            {"TeardropCleaver", "泪刃"},
            {"TerraEdge", "泰拉之锋"},
            {"TerraLance", "泰拉长枪"},
            {"Terratomere", "泰拉巨刃"},
            {"TerrorBlade", "惊惧魂刃"},
            {"TheBurningSky", "焚灭天惩"},
            {"TheEnforcer", "暴政"},
            {"TheEyeofCalamitas", "遗殇"},
            {"TheGodsGambit", "神旨"},
            {"TheLastMourning", "末晓之殇"},
            {"TheMicrowave", "微波辐照"},
            {"TheMutilator", "血石斩切刃"},
            {"TheObliterator", "混沌射线"},
            {"ThePlaguebringer", "瘟疫狂潮"},
            {"TitanArm", "泰坦之臂"},
            {"TriactisTruePaladinianMageHammerofMightMelee", "真·泰阿克提斯之圣骑士魔锤"},
            {"TrueArkoftheAncients", "远古方舟"},
            {"TrueBiomeBlade", "环境之刃"},
            {"TrueBloodyEdge", "真·血锋"},
            {"TrueCausticEdge", "真·绝望之刃"},
            {"TrueForbiddenOathblade", "真·誓约与禁忌之刃"},
            {"TruePaladinsHammerMelee", "堕落圣骑士之锤"},
            {"TrueTyrantYharimsUltisword", "真·暴君巨剑"},
            {"Tumbleweed", "风滚草"},
            {"TyphonsGreed", "贪魂台风"},
            {"TyrantYharimsUltisword", "暴君亚利姆之巨刃"},
            {"UltimusCleaver", "斩首者"},
            {"UrchinFlail", "海胆链球"},
            {"UrchinSpear", "赤潮长矛"},
            {"VeinBurster", "血肉之雷"},
            {"Verdant", "葱茏"},
            {"VirulentKatana", "瘟疫毒刃"},
            {"Whirlpool", "涡旋"},
            {"WindBlade", "风之刃"},
            {"WulfrumBlade", "钨钢刀"},
            {"XerocsGreatsword", "熵阔剑"},
            {"YateveoBloom", "绽花毒藤"},
            {"YinYo", "阴阳"},
            {"AcesHigh", "极魂符枪"},
            {"Alluvion", "啸流"},
            {"AMR", "反器材狙击步枪"},
            {"AngelicShotgun", "天使圣火炮"},
            {"Animosity", "憎恨"},
            {"AquashardShotgun", "碎水霰弹枪"},
            {"Arbalest", "劲弩"},
            {"Archerfish", "射水鱼"},
            {"ArterialAssault", "血星陨弓"},
            {"AstralBlaster", "幻星冲击炮"},
            {"AstralRepeater", "幻星弓"},
            {"AstrealDefeat", "星界"},
            {"Barinade", "巴淋内德"},
            {"Barinautical", "巴淋诺提卡"},
            {"BarracudaGun", "梭鱼枪"},
            {"BladedgeGreatbow", "锋叶十字弩"},
            {"BlissfulBombardier", "极乐升天炮"},
            {"BlossomFlux", "叶流"},
            {"BrimstoneFlamesprayer", "混乱之息"},
            {"BrimstoneFury", "硫火之怒"},
            {"Butcher", "屠夫"},
            {"ChickenCannon", "小鸡大炮"},
            {"ClamorRifle", "音波步枪"},
            {"ClaretCannon", "深红手炮"},
            {"CleansingBlaze", "净化怒焰"},
            {"ClockGatlignum", "发条鳄鱼枪"},
            {"ClockworkBow", "发条弓"},
            {"Contagion", "瘟疫"},
            {"CoralCannon", "珊瑚手炮"},
            {"CosmicBolter", "宇束弓"},
            {"CursedCapper", "诅咒地狱手枪"},
            {"DaemonsFlame", "鬼火弓"},
            {"DarkechoGreatbow", "暗之回响"},
            {"Deathwind", "死亡之风"},
            {"DodusHandcannon", "Dodu的手炮"},
            {"DragonsBreath", "巨龙之息"},
            {"Drataliornus", "赤陨霸龙弓"},
            {"EffluviumBow", "冰川之风"},
            {"ElementalBlaster", "元素冲击波"},
            {"ElementalEruption", "元素喷射器"},
            {"EternalBlizzard", "恒吹雪"},
            {"Eviscerator", "血肉喷射器"},
            {"FirestormCannon", "信号机关枪"},
            {"FlakKraken", "海妖克拉肯"},
            {"FlarewingBow", "黑翼蝙蝠弓"},
            {"FlurrystormCannon", "雪球机关炮"},
            {"FrostbiteBlaster", "霜牙冲击波"},
            {"Fungicide", "真菌枪"},
            {"Galeforce", "烈风"},
            {"GoldenEagle", "黄金之鹰"},
            {"Goobow", "凝胶弓"},
            {"GreatbowofTurmoil", "洲陆巨弓"},
            {"GunkShot", "胶泥散弹枪"},
            {"HalibutCannon", "大比目鱼炮"},
            {"HalleysInferno", "哈雷彗星炮"},
            {"HandheldTank", "手持坦克"},
            {"HeavenlyGale", "天堂之风"},
            {"Hellborn", "地狱降临"},
            {"Helstorm", "地狱风暴"},
            {"Infinity", "无限解离者"},
            {"Interfacer", "散播者"},
            {"Karasawa", "卡拉萨瓦重炮"},
            {"Leviatitan", "利维坦海潮枪"},
            {"LunarianBow", "月光弓"},
            {"MagnaStriker", "麦格纳冲锋炮"},
            {"MagnomalyCannon", "磁极异变"},
            {"Malevolence", "蜂毒"},
            {"MarniteRifleSpear", "花岗石刺枪"},
            {"Megafleet", "太虚星龙炮"},
            {"Megalodon", "巨舰鲨"},
            {"Meowthrower", "彩虹喵喷射器"},
            {"MepheticSprayer", "枯萎散播者"},
            {"MineralMortar", "岩鲨发射器"},
            {"Minigun", "速射机枪"},
            {"MolecularManipulator", "分子操纵器"},
            {"Monsoon", "季风"},
            {"Needler", "叶针枪"},
            {"NettlelineGreatbow", "荆棘藤"},
            {"Norfleet", "诺法雷"},
            {"NullificationRifle", "空"},
            {"OnyxChainBlaster", "玛瑙链炮"},
            {"Onyxia", "奥妮克希亚"},
            {"OpalStriker", "鹅卵石冲击波"},
            {"OverloadedBlaster", "过载凝胶发射器"},
            {"P90", "P90"},
            {"PaintballBlaster", "彩弹冲锋枪"},
            {"PearlGod", "珍珠之神"},
            {"PestilentDefiler", "感染者"},
            {"Phangasm", "幻魔"},
            {"Photoviscerator", "星火解离者"},
            {"PlanetaryAnnihilation", "湮灭千星"},
            {"PrideHuntersPlanarRipper", "雷空猎影"},
            {"Pumpler", "气泵枪"},
            {"RealmRavager", "灭域者"},
            {"SandstormGun", "沙尘暴"},
            {"Scorpion", "天蝎座"},
            {"SDFMG", "SDFMG"},
            {"Seabow", "芦苇吹筒"},
            {"Seadragon", "海龙"},
            {"SeasSearing", "海之烧灼"},
            {"SepticSkewer", "剧毒叉枪"},
            {"Shadethrower", "影流喷射器"},
            {"Shellshooter", "贝壳刃弓"},
            {"Shredder", "解离者"},
            {"Shroomer", "蘑菇狙击枪"},
            {"SlagMagnum", "熔渣马格南"},
            {"SomaPrime", "月神Prime"},
            {"SparkSpreader", "火花喷射器"},
            {"SpectralstormCannon", "光谱风暴"},
            {"SpectreRifle", "幽灵狙击枪"},
            {"Spyker", "叶针霰弹炮"},
            {"Starfleet", "星辰冲击波"},
            {"StarfleetMK2", "群星大炮"},
            {"StarSputter", "星辰溅散"},
            {"StellarCannon", "新星炮"},
            {"StormDragoon", "风暴龙骑士"},
            {"StormSurge", "旋风发射器"},
            {"SulphuricAcidCannon", "硫酸炮"},
            {"Svantechnical", "斯万科技枪"},
            {"TelluricGlare", "地核之炎"},
            {"TerraFlameburster", "泰拉喷火器"},
            {"TheBallista", "城弩"},
            {"TheEmpyrean", "神之风箱"},
            {"TheHive", "蜂巢发射器"},
            {"ThePack", "集束火箭"},
            {"TheStorm", "风暴"},
            {"TitaniumRailgun", "钛金电磁炮"},
            {"TrueConferenceCall", "电话会议"},
            {"Vortexpopper", "涡流泡泡枪"},
            {"WulfrumBow", "钨钢弓"},
            {"AccretionDisk", "元素飞盘"},
            {"AdamantiteThrowingAxe", "精金投斧"},
            {"AlphaVirus", "始源病毒"},
            {"AshenStalactite", "灰烬石钟乳"},
            {"BallisticPoisonBomb", "渊毒子母雷"},
            {"BlastBarrel", "炸药桶"},
            {"BlazingStar", "烈火之星"},
            {"BloodsoakedCrasher", "血沫重斧"},
            {"BouncingEyeball", "弹跳眼球"},
            {"BouncySpikyBall", "弹跳尖钉球"},
            {"BrackishFlask", "咸水瓶"},
            {"Brimblade", "硫火手里剑"},
            {"BurningStrife", "暗影炎尖钉球"},
            {"CelestialReaper", "星体掠夺者"},
            {"Celestus", "星神之杀"},
            {"Cinquedea", "五指剑"},
            {"CobaltKunai", "钴蓝苦无"},
            {"ConsecratedWater", "敬神圣水"},
            {"CorpusAvertor", "憎恶血刃"},
            {"CosmicKunai", "星体苦无"},
            {"CraniumSmasher", "碎颅飞盘"},
            {"CrushsawCrasher", "硫火飞爪"},
            {"Crystalline", "晶体飞刀"},
            {"CrystalPiercer", "水晶穿刺者"},
            {"CursedDagger", "咒焰匕首"},
            {"DeepSeaDumbbell", "渊海哑铃"},
            {"DeepWounder", "深渊裂斧"},
            {"DefectiveSphere", "致命缺陷"},
            {"DeificThunderbolt", "圣雷天劫"},
            {"DesecratedWater", "渎神邪水"},
            {"DukesDecapitator", "公爵斩首者"},
            {"DuneHopper", "越浪蛟"},
            {"DuststormInABottle", "瓶中尘"},
            {"EclipsesFall", "日蚀之陨"},
            {"EnchantedAxe", "附魔飞斧"},
            {"EpidemicShredder", "瘟疫解离者"},
            {"Equanimity", "宁静回旋镖"},
            {"Eradicator", "湮灭"},
            {"ExecutionersBlade", "终结者之刃"},
            {"Exorcism", "驱魔十字架"},
            {"FantasyTalisman", "魔幻符咒"},
            {"FeatherKnife", "羽刃"},
            {"FlameScythe", "板块斩切者"},
            {"FrostcrushValari", "霜碎回旋镖"},
            {"FrostyFlare", "霜冻引信"},
            {"GelDart", "凝胶飞镖"},
            {"GhoulishGouger", "狱卒之镰"},
            {"GildedDagger", "镀金匕首"},
            {"Glaive", "回飞刃"},
            {"GleamingDagger", "闪亮匕首"},
            {"GodsParanoia", "神明妄论"},
            {"HardenedHoneycomb", "硬化蜂巢块"},
            {"HeavenfallenStardisk", "堕天星盘"},
            {"HellsSun", "地底太阳"},
            {"Hypothermia", "急冻冰晶"},
            {"Icebreaker", "破冰者"},
            {"IceStar", "冰花"},
            {"IchorSpear", "腐脓标枪"},
            {"InfernalKris", "地狱飞剑"},
            {"IronFrancisca", "铁质飞斧"},
            {"JawsOfOblivion", "遗忘之颚"},
            {"KelvinCatalyst", "冰灵旋刃"},
            {"Kylie", "回飞棒"},
            {"LeadTomahawk", "铅质飞斧"},
            {"LeviathanTeeth", "利维坦之牙"},
            {"Lionfish", "狮子鱼"},
            {"LuminousStriker", "月影投矛"},
            {"LunarKunai", "月明苦无"},
            {"Malachite", "孔雀翎"},
            {"MangroveChakram", "红木环刃"},
            {"MetalMonstrosity", "易碎金属球"},
            {"MeteorFist", "陨星拳套"},
            {"MoltenAmputator", "熔岩切碎者"},
            {"MonkeyDarts", "猴子飞镖"},
            {"Mycoroot", "狂菌根系"},
            {"MythrilKnife", "秘银飞刀"},
            {"NanoblackReaperRogue", "纳米黑死神"},
            {"NastyCholla", "仙人掌尖钉球"},
            {"NightsGaze", "暗夜凝视"},
            {"Nychthemeron", "昼夜"},
            {"OrichalcumSpikedGemstone", "山铜宝石钉"},
            {"PalladiumJavelin", "钯金投矛"},
            {"Penumbra", "半影飞弹"},
            {"PhantasmalRuin", "幻魂归墟"},
            {"PhantomLance", "幻影投矛"},
            {"Plaguenade", "瘟疫蜜蜂手雷"},
            {"PoisonPack", "剧毒尖钉球"},
            {"Prismalline", "棱晶飞刀"},
            {"ProfanedPartisan", "渎神投矛"},
            {"ProfanedTrident", "狂龙翼火"},
            {"RadiantStar", "光辉星剑"},
            {"RegulusRiot", "星体暴乱"},
            {"RotBall", "腐蚀之球"},
            {"SandDollar", "沙钱"},
            {"Sandslasher", "沙暴手里剑"},
            {"ScarletDevil", "绯红恶魔"},
            {"ScourgeoftheDesert", "沙漠之灾"},
            {"ScourgeoftheSeas", "渊海之灾"},
            {"SeafoamBomb", "海泡石手雷"},
            {"SeashellBoomerang", "鱼骨回旋镖"},
            {"ShatteredSun", "裂阳"},
            {"ShinobiBlade", "忍者之刃"},
            {"ShockGrenade", "震爆弹"},
            {"Shroomerang", "真菌回旋镖"},
            {"SkyStabber", "天羽尖钉球"},
            {"SlickCane", "盗贼手杖"},
            {"SludgeSplotch", "污泥斑点"},
            {"SnapClam", "夹贝"},
            {"SpearofDestiny", "命运之矛"},
            {"SpearofPaleolith", "古岩投矛"},
            {"StarofDestruction", "毁灭之星"},
            {"StellarKnife", "恒星匕首"},
            {"StickySpikyBall", "粘性尖钉球"},
            {"StormfrontRazor", "风暴剃刃"},
            {"Supernova", "超新星"},
            {"TarragonThrowingDart", "龙蒿投镖"},
            {"TerraDisk", "泰拉飞盘"},
            {"TerrorTalons", "恐惧鹰爪"},
            {"TheReaper", "死神飞镰"},
            {"TheSyringe", "瘟疫注射器"},
            {"ThrowingBrick", "板砖"},
            {"TimeBolt", "时空交叉刃"},
            {"TitaniumShuriken", "钛金手里剑"},
            {"ToothBall", "尖牙簇"},
            {"TotalityBreakers", "集束燃爆瓶"},
            {"Turbulance", "乱流"},
            {"UrchinStinger", "深渊海胆蛰"},
            {"UtensilPoker", "百变餐叉"},
            {"Valediction", "辞别飞镰"},
            {"WebBall", "蛛网球"},
            {"WulfrumKnife", "钨钢飞刀"},
            {"XerocPitchfork", "半影裂片"},
            {"AbandonedSlimeStaff", "淡忘之史"},
            {"AncientIceChunk", "千载寒晶"},
            {"AngryChickenStaff", "巨龙七星灯"},
            {"BlackHawkRemote", "黑鹰遥控器"},
            {"BlightedEyeStaff", "混乱之祈祷"},
            {"BloodClotStaff", "血块法杖"},
            {"BorealisBomber", "极光爆破"},
            {"CalamarisLament", "乌贼之哀歌"},
            {"CorroslimeStaff", "腐化史莱姆召唤杖"},
            {"CosmicImmaterializer", "归虚之灵"},
            {"Cosmilamp", "无限之灯"},
            {"CrimslimeStaff", "血腥史莱姆召唤杖"},
            {"CryogenicStaff", "寒极法杖"},
            {"DankStaff", "沼泽之眼"},
            {"DeepseaStaff", "深海权杖"},
            {"DreadmineStaff", "恐惧渊雷法杖"},
            {"ElementalAxe", "元素之斧"},
            {"Endogenesis", "源生寒晶"},
            {"EnergyStaff", "能源法杖"},
            {"EtherealSubjugator", "幽花之主"},
            {"ForgottenApexWand", "失落极峰"},
            {"GodspawnHelixStaff", "星神圣护"},
            {"HerringStaff", "鲱鱼法杖"},
            {"HivePod", "星幻巢"},
            {"LanternoftheSoul", "遗境魂灯"},
            {"MagicalConch", "魔法海螺"},
            {"PlantationStaff", "苍华之庭"},
            {"ResurrectionButterfly", "反魂蝶"},
            {"SandSharknadoStaff", "沙龙卷法杖"},
            {"SeaboundStaff", "海星召唤杖"},
            {"ShellfishStaff", "海贝法杖"},
            {"Sirius", "天狼星"},
            {"SpikecragStaff", "地刺王法杖"},
            {"SquirrelSquireStaff", "松鼠召唤杖"},
            {"StaffoftheMechworm", "神明吞噬者召唤杖"},
            {"SunGodStaff", "太阳神法杖"},
            {"SunSpiritStaff", "太阳之灵法杖"},
            {"WulfrumController", "钨钢控制器"},
            {"Aestheticus", "美学魔杖"},
            {"AethersWhisper", "以太之低语"},
            {"Bazooka", "巴祖卡"},
            {"ElephantKiller", "猎象者"},
            {"Hydra", "九头蛇"},
            {"LightningHawk", "闪电鹰"},
            {"Magnum", "马格南"},
            {"GoldenGun", "灵液枪"},
            {"LunicEye", "星光之眼"},
            {"CorrosiveSpine", "腐蚀脊椎"},
            {"DukeScales", "硫海遗爵之鳞"},
            {"HallowedRune", "神圣符文"},
            {"MutatedTruffle", "突变松露"},
            {"NuclearRod", "核燃料棒"},
            {"Nucleogenesis", "核子之源"},
            {"ProfanedSoulCrystal", "渎神魂晶"},
            {"ScuttlersJewel", "潜遁者宝石"},
            {"StarbusterCore", "星爆核心"},
            {"StarTaintedGenerator", "玷星之源"},
            {"SilvaWings", "始源林海之翼"},
            {"BrimflameBoots", "硫火靴子"},
            {"BrimflameRobes", "硫火长袍"},
            {"BrimflameScowl", "硫火兜帽"},
            {"FathomSwarmerBoots", "幻渊鱼群护胫"},
            {"FathomSwarmerBreastplate", "幻渊鱼群胸甲"},
            {"FathomSwarmerVisage", "幻渊鱼群面罩"},
            {"SnowRuffianChestplate", "雪境暴徒胸甲"},
            {"SnowRuffianGreaves", "雪境暴徒护胫"},
            {"SnowRuffianMask", "雪境暴徒面具"},
            {"OldDukeMask", "硫海遗爵面具"},
            {"KnowledgeOldDuke", "硫海遗爵"},
            {"Acidwood", "酸蚀木"},
            {"CorrodedFossil", "酸腐化石"},
            {"SulfuricScale", "硫磺鳞片"},
            {"GeyserShell", "间歇泉贝壳"},
            {"SparksSummon", "附魔蝴蝶"},
            {"OldDukeTrophy", "硫海遗爵纪念章"},
            {"BloodwormItem", "血蠕虫"},
            {"CausticTear", "腐蚀之泪"},
            {"OldDukeBag", "宝藏袋"},
            {"GatlingLaser", "激光加特林"},
            {"LaserRifle", "重型激光步枪"},
            {"PlasmaCaster", "等离子液铸器"},
            {"PulseRifle", "脉冲步枪"},
            {"TeslaCannon", "特斯拉巨炮"},
            {"AcidGun", "酸液枪"},
            {"BelchingSaxophone", "喷嗝萨克斯"},
            {"Eternity", "恒"},
            {"Miasma", "瘴气"},
            {"SlitheringEels", "蜿蜒酸鳗"},
            {"VitriolicViper", "酸蚀毒蝰"},
            {"Basher", "痛击者"},
            {"Bonebreaker", "碎骨者"},
            {"SulphurousGrabber", "硫磺掠夺者"},
            {"BloodBoiler", "血祸沸炉"},
            {"FetidEmesis", "腐臭倾泻者"},
            {"FlakToxicannon", "剧毒高射炮"},
            {"PristineFury", "纯元怒焰"},
            {"Toxibow", "剧毒烈弓"},
            {"AcidicRainBarrel", "酸雨桶"},
            {"ContaminatedBile", "污秽胆汁"},
            {"SkyfinBombers", "天鳍毁灭者"},
            {"SpentFuelContainer", "辐毒废水"},
            {"ToxicantTwister", "剧毒旋风"},
            {"BelladonnaSpiritStaff", "莨菪之灵杖"},
            {"BensUmbrella", "光阴流时伞"},
            {"CadaverousCarrion", "干缩腐尸"},
            {"CausticCroakerStaff", "腐蚀蛙法杖"},
            {"CinderBlossomStaff", "烬英法杖"},
            {"ColdDivinity", "冰寒神性"},
            {"CorvidHarbringerStaff", "灭兆渡鸦杖"},
            {"CosmicViperEngine", "宙极毒蝰引擎"},
            {"DazzlingStabberStaff", "耀目穿刺者杖"},
            {"DormantBrimseeker", "沉眠寻焰者"},
            {"DragonbloodDisgorger", "龙血弃族"},
            {"EndoHydraStaff", "冰洋九头龙"},
            {"FrostBlossomStaff", "霜华法杖"},
            {"FuelCellBundle", "燃料电池组"},
            {"HauntedScroll", "闹鬼卷轴"},
            {"IgneousExaltation", "淬焰之升华"},
            {"InfectedRemote", "瘟疫控制器"},
            {"MidnightSunBeacon", "极昼信标"},
            {"OrthoceraShell", "直角石壳"},
            {"SarosPossession", "星律之握揽"},
            {"ScabRipper", "疤痕撕裂者"},
            {"StaffOfNecrosteocytes", "死灵骨髓杖"},
            {"StormjawStaff", "风暴蚁狮杖"},
            {"TacticalPlagueEngine", "战术瘟疫引擎"},
            {"VileFeeder", "腐化吞噬者"},
            {"BrokenWaterFilter", "破损净水器"},
            {"RustyMedallion", "锈蚀勋章"},
            {"PlagueCaller", "瘟疫呼机"},
            {"EffigyOfDecay", "腐朽塑像"},
            {"RustyChest", "锈蚀箱子"},
            {"ProfanedGuardianTrophy", "亵渎守卫纪念章"},
            {"SupremeCalamitasTrophy", "至尊灾厄纪念章"},
            {"HardenedSulphurousSandstone", "硬化硫磺沙岩"},
            {"HardenedSulphurousSandstoneWall", "硬化硫磺沙岩墙"},
            {"SulphurskinPotion", "硫磺皮肤药水"},
            {"CorrodedCaustibow", "酸腐弓"},
            {"GammaHeart", "伽马共生者"},
            {"RustyBeaconPrototype", "锈蚀信标原型"},
            {"StarStruckWater", "星水"},
            {"BabyGhostBellJar", "小鬼铃水母罐"},
            {"PiggyCage", "猪猪笼"},
            {"SeaMinnowJar", "海洋米诺鱼缸"},
            {"TwinklerInABottle", "星幻萤火虫瓶"},
            {"LivingBrimstoneFireBlock", "永燃硫磺火"},
            {"LivingGodSlayerFireBlock", "永燃弑神火"},
            {"LivingHolyFireBlock", "永燃圣火"},
            {"RelicOfConvergence", "聚魂圣物"},
            {"RelicOfResilience", "塑魂圣物"},
            {"ForbiddenCirclet", "禁戒金箍"},
            {"AstralBlueDye", "星辉之蓝染料"},
            {"AstralOrangeDye", "星辉之橙染料"},
            {"AstralSwirlDye", "融星染料"},
            {"CeaselessDye", "无尽空虚染料"},
            {"DefiledFlameDye", "神殇烈焰染料"},
            {"DragonSoulDye", "神龙之魂染料"},
            {"ElementalDye", "元素涌动染料"},
            {"EndothermicDye", "恒温之刻染料"},
            {"AdrenalineHairDye", "肾上腺素染发剂"},
            {"RageHairDye", "怒气染发剂"},
            {"StealthHairDye", "潜伏染发剂"},
            {"WingTimeHairDye", "飞行时间染发剂"},
            {"NightmareDye", "万圣梦魇染料"},
            {"ReaverDye", "掠夺之恒染料"},
            {"ShadowspecDye", "灾劫魔影染料"},
            {"StratusDye", "幻空流云染料"},
            {"JoyfulHeart", "欢愉之心"},
            {"AbyssFountainItem", "深渊喷泉"},
            {"AstralFountainItem", "星辉喷泉"},
            {"SulphurousFountainItem", "硫磺喷泉"},
            {"SunkenSeaFountain", "沉沦喷泉"},
            {"ValkyrieRay", "女武神权杖"},
            {"LionHeart", "狮心圣裁"},
            {"TundraFlameBlossomsStaff", "永冻之焱华"},
            {"RelicOfDeliverance", "渡魂圣物"},
            {"BlunderBooster", "悖论增幅器"},
            {"EvasionScarf", "闪避围巾"},
            {"HowlsHeart", "哈尔之心"},
            {"RoverDrive", "钨钢屏障生成仪"},
            {"DesertProwlerHat", "沙漠巡游者帽"},
            {"DesertProwlerPants", "沙漠巡游者裤"},
            {"DesertProwlerShirt", "沙漠巡游者衣"},
            {"PlaguebringerCarapace", "瘟疫使者铠甲"},
            {"PlaguebringerPistons", "瘟疫使者推进靴"},
            {"PlaguebringerVisor", "瘟疫使者面具"},
            {"PrismaticGreaves", "光棱护胫"},
            {"PrismaticHelmet", "光棱头盔"},
            {"PrismaticRegalia", "光棱斗篷"},
            {"TitanHeartBoots", "泰坦之心靴子"},
            {"TitanHeartMantle", "泰坦之心斗篷"},
            {"TitanHeartMask", "泰坦之心面具"},
            {"AnahitaMask", "阿娜希塔面具"},
            {"DraedonsLogHell", "嘉登日志 - 地狱"},
            {"DraedonsLogJungle", "嘉登日志 - 丛林与瘟疫"},
            {"DraedonsLogPlanetoid", "嘉登日志 - 太空"},
            {"DraedonsLogSnowBiome", "嘉登日志 - 永冻废土"},
            {"DraedonsLogSunkenSea", "嘉登日志 - 对沉沦海水生生命的研究记录"},
            {"ProfanedMoonlightDye", "亵渎月色染料"},
            {"DubiousPlating", "可疑镀层"},
            {"EnergyCore", "能源核心"},
            {"InfectedArmorPlating", "瘟疫装甲镀层"},
            {"MysteriousCircuitry", "神秘电路"},
            {"TitanHeart", "泰坦之心"},
            {"BirdSeed", "痴愚饲料"},
            {"ChromaticOrb", "彩球"},
            {"AgedLaboratoryConsoleItem", "老旧实验室控制台"},
            {"AgedLaboratoryContainmentBoxItem", "老旧实验室密封箱"},
            {"AgedLaboratoryDisplayItem", "老旧实验室展柜"},
            {"AgedLaboratoryDoorItem", "老旧实验室大门"},
            {"AgedLaboratoryElectricPanelItem", "老旧实验室配电盘"},
            {"AgedLaboratoryScreenItem", "老旧实验室荧幕"},
            {"AgedLaboratoryServerItem", "老旧实验室服务器"},
            {"AgedLaboratoryTerminalItem", "老旧实验室终端机"},
            {"AgedReinforcedCrateItem", "老旧实验室板条箱"},
            {"HazardChevronPanels", "警示面板"},
            {"LaboratoryConsoleItem", "实验室控制台"},
            {"LaboratoryContainmentBoxItem", "实验室密封箱"},
            {"LaboratoryDisplayItem", "实验室展柜"},
            {"LaboratoryDoorItem", "实验室大门"},
            {"LaboratoryElectricPanelItem", "实验室配电盘"},
            {"LaboratoryPanels", "实验室板块"},
            {"LaboratoryPipePlating", "实验室电镀管道"},
            {"LaboratoryPlating", "实验室搁板"},
            {"LaboratoryScreenItem", "实验室荧幕"},
            {"LaboratoryServerItem", "实验室服务器"},
            {"LaboratoryShelf", "实验室架"},
            {"LaboratoryTerminalItem", "实验室终端机"},
            {"ReinforcedCrateItem", "加固板条箱"},
            {"RustedPipes", "生锈管道"},
            {"RustedPlating", "生锈镀层"},
            {"RustedShelf", "生锈架"},
            {"AgedSecurityChest", "老旧安全箱"},
            {"AstralBeaconItem", "星辉信标"},
            {"AuricToilet", "圣金马桶"},
            {"SecurityChest", "安全箱"},
            {"AnahitaTrophy", "阿娜希塔纪念章"},
            {"Chaosplate", "混沌石"},
            {"Elumplate", "永冻石"},
            {"Navyplate", "沉沦石"},
            {"PlagueContainmentCells", "疫病石"},
            {"ChaosplateWall", "混沌石墙"},
            {"HazardChevronWall", "警戒墙"},
            {"LaboratoryPanelWall", "实验室板块墙"},
            {"LaboratoryPlateBeam", "实验室板梁"},
            {"LaboratoryPlatePillar", "实验室板柱"},
            {"LaboratoryPlatingWall", "实验室板墙"},
            {"RustedPlateBeam", "生锈板梁"},
            {"RustedPlatePillar", "生锈板柱"},
            {"RustedPlatingWall", "生锈板墙"},
            {"ElumplateWall", "永冻石墙"},
            {"NavyplateWall", "沉沦石墙"},
            {"PlagueContainmentCellsWall", "疫病石墙"},
            {"ShadowPotion", "暗影药剂"},
            {"PowerCell", "嘉登能源电池"},
            {"AbyssalTreasure", "深渊宝藏"},
            {"BleachedAnglingKit", "褪色钓鱼箱"},
            {"FleshyGeodeT1", "血肉晶簇"},
            {"FleshyGeodeT2", "死灵晶簇"},
            {"SandyAnglingKit", "砂质钓鱼箱"},
            {"SulphuricTreasure", "硫磺宝藏"},
            {"FrequencyManipulator", "变频器"},
            {"GalvanizingGlaive", "磁能分割刀"},
            {"GaussDagger", "高斯短匕"},
            {"GaussPistol", "高斯手枪"},
            {"GaussRifle", "高斯来复枪"},
            {"HydraulicVoltCrasher", "液压伏冲器"},
            {"MatterModulator", "物质调谐器"},
            {"MountedScanner", "架式扫射机"},
            {"Phaseslayer", "相位剑"},
            {"PlasmaGrenade", "等离子手雷"},
            {"PoleWarper", "电极解离机"},
            {"PulseDragon", "脉冲龙链枷"},
            {"PulsePistol", "脉冲手枪"},
            {"PulseTurretRemote", "脉冲炮塔遥控装置"},
            {"SnakeEyes", "蛇眼"},
            {"StarSwallowerContainmentUnit", "噬星者控制元件"},
            {"SystemBane", "神经系统紊乱装置"},
            {"Taser", "泰瑟枪"},
            {"TrackingDisk", "逐影飞盘"},
            {"WavePounder", "震波炸弹"},
            {"AlulaAustralis", "南极光羽"},
            {"ParasiticSceptor", "寄生权杖"},
            {"MonstrousKnives", "畸变飞刀"},
            {"PrismaticBreaker", "光棱破碎者"},
            {"TenebreusTides", "深渊潮涌"},
            {"AuroraBlazer", "极光烈焰"},
            {"MarksmanBow", "猎人长弓"},
            {"RubicoPrime", "绝路Prime"},
            {"TyrannysEnd", "暴政之终"},
            {"Ultima", "创世纪"},
            {"Apoctolith", "末渊石板"},
            {"AuroradicalThrow", "源野天光"},
            {"FinalDawn", "终曲黎明"},
            {"LeonidProgenitor", "狮源流星"},
            {"SealedSingularity", "秘封奇点"},
            {"CausticStaff", "蚀骨法杖"},
            {"GastricBelcherStaff", "畸变吞食者法杖"},
            {"PrototypeAndromechaRing", "月陨天环"},
            {"YanmeisKnife", "Yanmei的刀"},
            {"AscendantSpiritEssence", "化神魂精"},
            {"AeroDynamite", "天蓝炸药"},
            {"BloodfireBullet", "血炎弹"},
            {"GodSlayerSlug", "弑神者弹"},
            {"ChargingStationItem", "充能站"},
            {"LabHologramProjectorItem", "实验室全息投影仪"},
            {"PowerCellFactoryItem", "燃料电池工厂"},
            {"BallAndChain", "球镣"},
            {"Auralis", "极光"},
            {"AngelicAlliance", "圣天誓盟"},
            {"Calamity", "灾厄"},
            {"DaawnlightSpiritOrigin", "曙旼始灵"},
            {"PhantomicArtifact", "幻魂神物"},
            {"RottenDogtooth", "腐烂犬齿"},
            {"SandSharkToothNecklace", "旱海鲨牙项链"},
            {"ShatteredCommunity", "幻灭心元石"},
            {"AntiCystOintment", "抗囊肿药膏"},
            {"AntiTumorOintment", "抗肿瘤药膏"},
            {"ReaverHeadgear", "掠夺者头饰"},
            {"ApolloMask", "阿波罗面具"},
            {"AresMask", "阿瑞斯面具"},
            {"ArtemisMask", "阿尔忒弥斯面具"},
            {"AshenHorns", "余烬之角"},
            {"DraedonMask", "嘉登面具"},
            {"SCalBoots", "灾厄魔靴"},
            {"SCalMask", "灾厄兜帽"},
            {"SCalRobes", "灾厄长袍"},
            {"ThanatosMask", "塔纳托斯面具"},
            {"BleachBall", "白色水球"},
            {"BrimstoneLocus", "硫火魔珠"},
            {"Malice", "恶意之脑"},
            {"AdvancedDisplay", "高级显示屏"},
            {"AuricQuantumCoolingCell", "金源量子冷却电池"},
            {"CodebreakerBase", "密码破译器基站"},
            {"DecryptionComputer", "解密计算机"},
            {"EncryptedSchematicHell", "加密原型图"},
            {"EncryptedSchematicIce", "加密原型图"},
            {"EncryptedSchematicJungle", "加密原型图"},
            {"EncryptedSchematicPlanetoid", "加密原型图"},
            {"EncryptedSchematicSunkenSea", "原型图"},
            {"LongRangedSensorArray", "远程传感器阵列"},
            {"VoltageRegulationSystem", "电压校准系统"},
            {"AerialiteDye", "苍青辉蓝染料"},
            {"AstralDye", "幻星之魄染料"},
            {"AuricDye", "圣金源染料"},
            {"BloodflareDye", "沸血炽炎染料"},
            {"BlueCosmicFlameDye", "瀚蓝宙焰染料"},
            {"BlueStatigelDye", "蓝色斯塔提斯染料"},
            {"BrimflameDye", "硫火染料"},
            {"CalamitousDye", "至尊灾厄染料"},
            {"CosmiliteDye", "恒宇弘宙染料"},
            {"CryonicDye", "寒元辉光染料"},
            {"ExoDye", "星宇流霆染料"},
            {"PhantoplasmDye", "幽喑幻魂染料"},
            {"PinkCosmicFlameDye", "霏粉宙焰染料"},
            {"PinkStatigelDye", "粉色斯塔提斯染料"},
            {"ProfanedFlameDye", "渎神圣火染料"},
            {"SlimeGodDye", "史莱姆之神染料"},
            {"SwirlingCosmicFlameDye", "宙宇潆焰染料"},
            {"CyanSeekingMechanism", "海蓝色定位仪"},
            {"GreenSeekingMechanism", "绿色定位仪"},
            {"MysteriousMechanism", "实验室定位仪"},
            {"PurpleSeekingMechanism", "紫色定位仪"},
            {"RedSeekingMechanism", "红色定位仪"},
            {"YellowSeekingMechanism", "黄色定位仪"},
            {"KnowledgeExoMechs", "星流巨械"},
            {"ExoPrism", "星流棱晶"},
            {"MiracleMatter", "奇迹物质"},
            {"ExoThrone", "星流之盒"},
            {"DoGCart", "神明矿车"},
            {"LittleLight", "机灵"},
            {"PineapplePet", "菠萝"},
            {"RottingEyeball", "腐烂眼球"},
            {"CosmicAnvilItem", "星宇砧"},
            {"ThaumaticChair", "奇迹之椅"},
            {"ApolloTrophy", "阿波罗纪念章"},
            {"AresTrophy", "阿瑞斯纪念章"},
            {"ArtemisTrophy", "阿尔忒弥斯纪念章"},
            {"ThanatosTrophy", "塔纳托斯纪念章"},
            {"StratusStarPlatformItem", "天星平台"},
            {"HallowedOre", "神圣矿"},
            {"SCalAltarItem", "诅咒祭坛"},
            {"SirenproofEarMuffs", "防海妖耳塞"},
            {"MartianDistressBeacon", "火星危机信标"},
            {"CeaselessVoidBag", "宝藏袋"},
            {"DraedonTreasureBag", "宝藏保险箱"},
            {"SCalBag", "宝藏保险箱"},
            {"SignusBag", "宝藏袋"},
            {"StormWeaverBag", "宝藏袋"},
            {"VoodooDemonVoodooDoll", "巫毒恶魔的巫毒娃娃"},
            {"GruesomeEminence", "异端僭越"},
            {"Heresy", "邪崇"},
            {"RainbowPartyCannon", "彩虹派对加农炮"},
            {"Rancor", "怨戾"},
            {"PhosphorescentGauntlet", "磷光拳套"},
            {"PhotonRipper", "光子撕裂者"},
            {"RemsRevenge", "雷姆的复仇"},
            {"SpineOfThanatos", "塔纳托斯的脊椎"},
            {"Violence", "恣睢"},
            {"BulletFilledShotgun", "满弹霰弹枪"},
            {"Condemnation", "诘责"},
            {"SurgeDriver", "煌晖御流炮"},
            {"TheJailor", "星耦狱锁"},
            {"RefractionRotor", "衍曜旋刃"},
            {"Sacrifice", "牺牲"},
            {"TheAtomSplitter", "微元劫裂者"},
            {"CindersOfLament", "余烬挽歌"},
            {"DaedalusGolemStaff", "代达罗斯守卫法杖"},
            {"DeathstareRod", "死亡凝视法杖"},
            {"EyeOfNight", "永夜之眼"},
            {"FleshOfInfidelity", "不洁血肉"},
            {"FlowersOfMortality", "告死之花"},
            {"Metastasis", "硫心恶殖"},
            {"Perdition", "永劫"},
            {"PolypLauncher", "珊瑚召唤杖"},
            {"SlimePuppetStaff", "史莱姆傀儡法杖"},
            {"UniverseSplitter", "渊宇天基炮"},
            {"Vigilance", "恂戒"},
            {"ViralSprout", "剧毒芽孢"},
            {"ViridVanguard", "葱茏之锋"},
            {"VoidConcentrationStaff", "幽冥统御"},
            {"WitherBlossomsStaff", "凋零绽放法杖"},
            {"SupremeCataclysmTrophy", "至尊灾祸纪念章"},
            {"SupremeCatastropheTrophy", "至尊灾难纪念章"},
            {"OccultSkullCrown", "玄秘颅冠"},
            {"CirrusDress", "醉仙女的小裙子"},
            {"GemTechBodyArmor", "星钻胸甲"},
            {"GemTechHeadgear", "星钻头饰"},
            {"GemTechSchynbaulds", "星钻战裙"},
            {"CalamityHood", "灾厄的兜帽"},
            {"CalamityRobes", "灾厄的长袍"},
            {"PlasmaDriveCore", "等离子驱动核心原型"},
            {"SuspiciousScrap", "可疑的电路板"},
            {"McNuggets", "金拱门炸鸡块"},
            {"AcidwoodBathtub", "酸腐木浴缸"},
            {"AcidwoodBed", "酸腐木床"},
            {"AcidwoodBench", "酸腐木长凳"},
            {"AcidwoodBookcase", "酸腐木书架"},
            {"AcidwoodCandelabra", "酸腐木烛台"},
            {"AcidwoodCandle", "酸腐木蜡烛"},
            {"AcidwoodChair", "酸腐木椅子"},
            {"AcidwoodChandelier", "酸腐木吊灯"},
            {"AcidwoodChest", "酸腐木箱"},
            {"AcidwoodClock", "酸腐木钟"},
            {"AcidwoodDoor", "酸腐木门"},
            {"AcidwoodDresser", "酸腐木梳妆台"},
            {"AcidwoodLamp", "酸腐木灯"},
            {"AcidwoodLantern", "酸腐木灯笼"},
            {"AcidwoodPiano", "酸腐木钢琴"},
            {"AcidwoodPlatform", "酸腐木平台"},
            {"AcidwoodSink", "酸腐木水槽"},
            {"AcidwoodTable", "酸腐木桌子"},
            {"AcidwoodWorkBench", "酸腐木工作台"},
            {"ExoBathtub", "星流浴缸"},
            {"ExoBed", "星流床"},
            {"ExoBookcase", "星流书架"},
            {"ExoCandelabra", "星流烛台"},
            {"ExoCandle", "星流蜡烛"},
            {"ExoChair", "星流椅子"},
            {"ExoChandelier", "星流吊灯"},
            {"ExoChest", "星流箱"},
            {"ExoClock", "星流钟"},
            {"ExoConsole", "星流控制台"},
            {"ExoDisplay", "星流展柜"},
            {"ExoDoor", "星流门"},
            {"ExoDresser", "星流梳妆台"},
            {"ExoKeyboard", "星流键盘"},
            {"ExoLamp", "星流灯"},
            {"ExoLantern", "星流灯笼"},
            {"ExoPlatform", "星流平台"},
            {"ExoPlating", "星流镶板"},
            {"ExoPrismPanel", "星流棱晶镶板"},
            {"ExoPrismPlatform", "星流棱晶平台"},
            {"ExoScreen", "星流荧幕"},
            {"ExoServer", "星流服务器"},
            {"ExoSink", "星流水槽"},
            {"ExoSofa", "星流沙发"},
            {"ExoTable", "星流桌"},
            {"ExoTerminal", "星流终端机"},
            {"ExoToilet", "星流马桶"},
            {"ExoWorkbench", "星流工作台"},
            {"ExoPlatingWallItem", "星流镶板墙"},
            {"ExoPrismPanelWallItem", "星流棱晶镶板墙"},
            {"ArtAttack", "艺术攻击"},
            {"GloriousEnd", "荣耀尽头"},
            {"TacticiansTrumpCard", "战术家的王牌"},
            {"ThePrince", "“王子”"},
            {"ScorchedEarth", "焦土"},
            {"UniversalGenesis", "宇宙之源"},
            {"SearedPan", "焦锅"},
            {"Seraphim", "炽天使之匕"},
			{"AquaticScourgeRelic", "渊海灾虫圣物"},
			{"AstrumAureusRelic", "白金星舰圣物"},
			{"AstrumDeusRelic", "星神游龙圣物"},
			{"BrimstoneElementalRelic", "硫磺火元素圣物"},
			{"CalamitasCloneRelic", "灾厄圣物"},
			{"CalamitasRelic", "至尊灾厄圣物"},
			{"CeaselessVoidRelic", "无尽虚空圣物"},
			{"CrabulonRelic", "菌生蟹圣物"},
			{"CragmawMireRelic", "峭咽潭圣物"},
			{"CryogenRelic", "极地之灵圣物"},
			{"DesertScourgeRelic", "荒漠灾虫圣物"},
			{"DevourerOfGodsRelic", "神明吞噬者圣物"},
			{"DraedonRelic", "嘉登圣物"},
			{"DragonfollyRelic", "痴愚金龙圣物"},
			{"GiantClamRelic", "巨像蛤圣物"},
			{"GreatSandSharkRelic", "旱海狂鲨圣物"},
			{"HiveMindRelic", "腐巢意志圣物"},
			{"LeviathanAnahitaRelic", "利维坦&阿娜希塔圣物"},
			{"MaulerRelic", "渊海狂鲨圣物"},
			{"NuclearTerrorRelic", "辐射骇兽圣物"},
			{"OldDukeRelic", "硫海遗爵圣物"},
			{"PerforatorsRelic", "血肉宿主圣物"},
			{"PlaguebringerGoliathRelic", "瘟疫使者歌莉娅圣物"},
			{"PolterghastRelic", "噬魂幽花圣物"},
			{"ProfanedGuardiansRelic", "亵渎守卫圣物"},
			{"ProvidenceRelic", "亵渎天神圣物"},
			{"RavagerRelic", "毁灭魔像圣物"},
			{"SignusRelic", "神之使徒西格纳斯圣物"},
			{"SlimeGodRelic", "史莱姆之神圣物"},
			{"StormWeaverRelic", "风暴编织者圣物"},
			{"YharonRelic", "丛林龙，犽戎圣物"},

        };
    }
}
