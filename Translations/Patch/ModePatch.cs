...
namespace CalamityCN.Translations.Patch
{
    public class ModePatch
    {
        ...
        public static void Load()
        {
            ...
            {
                new string[2]{":Enables the][c/85FFE9: Adrenaline] [c/",
                    ":启用][c/85FFE9: 肾上腺素] [c/".zh()},
                new string[2]{":mechanic. You gain Adrenaline while fighting bosses. Getting hit drops Adrenaline back to 0] \n[c/",
                    ":机制. 对战Boss时可积攒肾上腺素条，受击时肾上腺素条归零。] \n[c/".zh()},
                new string[2]{":    When Adrenaline is maxed, press] ",
                    ":    当肾上腺素条满时，按下] ".zh()},
                new string[2]{":for a large damage boost] \n[c/",
                    ":获得大量伤害提升。] \n[c/".zh()},
                new string[2]{":Enables the ][c/FF8B54:Rage][c/",
                    ":启用][c/FF8B54: 怒气][c/".zh()},
                new string[2]{": mechanic. You gain Rage when in proximity of enemies or by using certain items] \n[c/",
                    ": 机制. 在接近敌人或使用特定物品时会积攒怒气条。] \n[c/".zh()},
                new string[2]{":    When Rage is maxed, press] ",
                    ":    当怒气条满时，按下] ".zh()},
            ...
            foreach (string[] il in Mode1)
            {
                if (!il[1].Equals(""))
                {
                    QuickTranslate(typeof(RevengeanceDifficulty), il[0], il[1]);
                }
            }
