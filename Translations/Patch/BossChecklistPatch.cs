...
namespace CalamityCN.Translations.Patch
{
	...
	public class BossChecklistPatch : OnPatcher
	{
		...
		public override Delegate Delegate
		{
			get
			{
				return new Action<BossChecklistPatch.AddCalamityBossesDelegate, Mod, Mod>(this.Translation);
			}
		}

		private void Translation(BossChecklistPatch.AddCalamityBossesDelegate orig, Mod bossChecklist, Mod calamity)
		{
			string name = "荒漠灾虫";
			...
			BossChecklistPatch.AddBoss(bossChecklist, calamity, name, difficulty, list, downed, summon, list2, "在沙漠环境使用 [i:" + ModContent.ItemType<DesertMedallion>().ToString() + "] 召唤", CalamityUtils.ColorMessage("荒漠灾虫潜入旱海之中……", new Color(238, 232, 170)), () => true, delegate (SpriteBatch sb, Rectangle rect, Color color)
			{
			...
			BossChecklistPatch.AddMiniBoss(bossChecklist, calamity, "巨像蛤", 1.61f, ModContent.NPCType<GiantClam>(), () => DownedBossSystem.downedCLAM, null, null, "荒漠灾虫后自然刷新在沉沦之海", CalamityUtils.ColorMessage("巨像蛤重返水下岩窟之中……", new Color(127, 255, 212)), () => true, null, null);
			string name2 = "菌生蟹";
			...
			BossChecklistPatch.AddBoss(bossChecklist, calamity, name2, difficulty2, npcType, downed2, summon2, list3, "在发光蘑菇地使用 [i:" + ModContent.ItemType<DecapoditaSprout>().ToString() + "] 召唤", CalamityUtils.ColorMessage("菌生蟹失去了对你的兴趣……", new Color(100, 149, 237)), () => true, null, null);
			string name3 = "腐巢意志";
			...
			BossChecklistPatch.AddBoss(bossChecklist, calamity, name3, difficulty3, npcType2, downed3, summon3, list4, "杀死刷新在腐化之地的腐化囊或在腐化之地使用 [i:" + ModContent.ItemType<Teratoma>().ToString() + "] 召唤", CalamityUtils.ColorMessage("腐化之源开始搜寻下一处繁衍之所……", new Color(148, 0, 211)), () => true, null, "CalamityMod/NPCs/HiveMind/HiveMindP2_Head_Boss");
			string name4 = "血肉宿主";
			...
			BossChecklistPatch.AddBoss(bossChecklist, calamity, name4, difficulty4, npcType3, downed4, summon4, list5, "杀死刷新在猩红之地的血肉囊或在猩红之地使用 [i:" + ModContent.ItemType<BloodyWormFood>().ToString() + "] 召唤", CalamityUtils.ColorMessage("寄生意志开始搜寻下一位不幸宿主……", new Color(220, 20, 60)), () => true, null, null);
			string name5 = "史莱姆之神";
			...
			BossChecklistPatch.AddBoss(bossChecklist, calamity, name5, difficulty5, list6, downed5, summon5, list7, "使用 [i:" + ModContent.ItemType<OverloadedSludge>().ToString() + " ] 召唤", CalamityUtils.ColorMessage("归元凝一的丑恶存在为它的同胞们完成了复仇……", new Color(186, 85, 51)), () => true, null, null);
			string name6 = "极地之灵";
			...
			BossChecklistPatch.AddBoss(bossChecklist, calamity, name6, difficulty6, npcType4, downed6, summon6, list8, "在雪原群系使用 [i:" + ModContent.ItemType<CryoKey>().ToString() + "] 召唤", CalamityUtils.ColorMessage("冰灵在冷冽彻骨的寒风中渐行渐远……", new Color(0, 255, 255)), () => true, null, "CalamityMod/NPCs/Cryogen/Cryogen_Phase1_Head_Boss");
			string name7 = "渊海灾虫";
			...
			BossChecklistPatch.AddBoss(bossChecklist, calamity, name7, difficulty7, list9, downed7, summon7, list10, "在硫磺海使用 [i:" + ModContent.ItemType<Seafood>().ToString() + "] 召唤，或在硫磺海等待其自然刷新", CalamityUtils.ColorMessage("渊海灾虫游回汪洋之中……", new Color(238, 232, 170)), () => true, delegate (SpriteBatch sb, Rectangle rect, Color color)
			{
			...
			BossChecklistPatch.AddBoss(bossChecklist, calamity, name8, difficulty8, npcType5, downed8, summon8, list11, "在硫火之崖使用 [i:" + ModContent.ItemType<CharredIdol>().ToString() + " ] 召唤", CalamityUtils.ColorMessage("硫磺火元素重返她曾经的圣殿，如今的废墟中……", new Color(220, 20, 60)), () => true, null, null);
			string name9 = "灾厄";
			...
			BossChecklistPatch.AddBoss(bossChecklist, calamity, name9, difficulty9, npcType6, downed9, summon9, list12, "在夜晚使用 [i:" + ModContent.ItemType<EyeofDesolation>().ToString() + " ] 召唤", CalamityUtils.ColorMessage("如果你渴望一场真正的战斗，那你战斗之前应当多做准备……", new Color(255, 165, 0)), () => true, null, null);
			string name10 = "旱海狂鲨";
			...
			list13.Add(3796);
			BossChecklistPatch.AddMiniBoss(bossChecklist, calamity, name10, difficulty10, npcType7, downed10, summon10, list13, "在花后沙尘暴杀死10只沙鲨或在沙漠使用 [i:" + ModContent.ItemType<SandstormsCore>().ToString() + " 召唤", CalamityUtils.ColorMessage("终极掠食者隐匿到沙丘之下……", new Color(218, 165, 32)), () => true, null, null);
			...
			BossChecklistPatch.AddBoss(bossChecklist, calamity, name11, difficulty11, list14, downed11, summon11, list15, "杀死海洋中的神秘生物", CalamityUtils.ColorMessage("海中恐惧沉入大洋之底……", new Color(127, 255, 212)), () => true, delegate (SpriteBatch sb, Rectangle rect, Color color)
			{
			...
			BossChecklistPatch.AddBoss(bossChecklist, calamity, name12, difficulty12, npcType8, downed12, summon12, list16, "在星辉瘟疫的夜晚使用 [i:" + ModContent.ItemType<AstralChunk>().ToString() + "] 召唤", CalamityUtils.ColorMessage("白金星舰指令已执行，开始返回……", new Color(255, 215, 0)), () => true, delegate (SpriteBatch sb, Rectangle rect, Color color)
			{
			...
			BossChecklistPatch.AddBoss(bossChecklist, calamity, name13, difficulty13, npcType9, downed13, summon13, list17, "在丛林使用 [i:" + ModContent.ItemType<Abombination>().ToString() + "] 召唤", CalamityUtils.ColorMessage("威胁性实体已被抹除，启动归巢程序。", new Color(0, 255, 0)), () => true, delegate (SpriteBatch sb, Rectangle rect, Color color)
			{
			...
			BossChecklistPatch.AddBoss(bossChecklist, calamity, name14, difficulty14, list18, downed14, summon14, list19, "使用 [i:" + ModContent.ItemType<DeathWhistle>().ToString() + "] 召唤", CalamityUtils.ColorMessage("那由暴君所屠戮之人的血肉，扭曲形成的战争巨兽，再次开始寻找真正的恶徒……", new Color(178, 34, 34)), () => true, delegate (SpriteBatch sb, Rectangle rect, Color color)
			{
			...
			BossChecklistPatch.AddBoss(bossChecklist, calamity, name15, difficulty15, list20, downed15, list21, list22, string.Concat(new string[]
			{
				"使用 [i:",
			...
			BossChecklistPatch.AddBoss(bossChecklist, calamity, name16, difficulty16, npcType10, downed16, summon15, list23, "在白天的神圣之地或地狱使用 [i:" + ModContent.ItemType<ProfanedShard>().ToString() + "] 召唤", CalamityUtils.ColorMessage("亵渎守卫必将保卫他们的至高女神，在所不辞……", new Color(255, 165, 0)), () => true, delegate (SpriteBatch sb, Rectangle rect, Color color)
			{
			...
			BossChecklistPatch.AddBoss(bossChecklist, calamity, name17, difficulty17, npcType11, downed17, summon16, list24, "在丛林使用 [i:" + ModContent.ItemType<ExoticPheromones>().ToString() + "] 召唤", CalamityUtils.ColorMessage("失败实验体重启繁衍日程……", new Color(255, 215, 0)), () => true, null, null);
			string name18 = "亵渎天神";
			...
			BossChecklistPatch.AddBoss(bossChecklist, calamity, name18, difficulty18, npcType12, downed18, summon17, list25, "在白天的神圣之地或地狱使用 [i:" + ModContent.ItemType<ProfanedCore>().ToString() + "] 召唤", CalamityUtils.ColorMessage("亵渎天神隐没在耀眼火光中……", new Color(255, 165, 0)), () => true, delegate (SpriteBatch sb, Rectangle rect, Color color)
			{
			...
			BossChecklistPatch.AddBoss(bossChecklist, calamity, name19, difficulty19, npcType13, downed19, summon18, list26, "在地牢使用 [i:" + ModContent.ItemType<RuneofKos>().ToString() + "] 召唤", CalamityUtils.ColorMessage("时空间的裂隙从你所能触及之处隐去……", new Color(75, 0, 130)), () => true, null, null);
			string name20 = "风暴编织者";
			...
			BossChecklistPatch.AddBoss(bossChecklist, calamity, name20, difficulty20, list27, downed20, summon19, list28, "在太空使用 [i:" + ModContent.ItemType<RuneofKos>().ToString() + "] 召唤", CalamityUtils.ColorMessage("风暴编织者再度遁入风暴之中……", new Color(238, 130, 238)), () => true, delegate (SpriteBatch sb, Rectangle rect, Color color)
			{
			...
			BossChecklistPatch.AddBoss(bossChecklist, calamity, name21, difficulty21, npcType14, downed21, summon20, list29, "在地狱使用 [i:" + ModContent.ItemType<RuneofKos>().ToString() + "] 召唤", CalamityUtils.ColorMessage("神使干净利落地完成了暗杀，不费吹灰之力……", new Color(186, 85, 211)), () => true, null, null);
			string name22 = "噬魂幽花";
			...
			BossChecklistPatch.AddBoss(bossChecklist, calamity, name22, difficulty22, list30, downed22, summon21, list31, "杀死30只幻魂或在地牢使用 [i:" + ModContent.ItemType<NecroplasmicBeacon>().ToString() + "] 召唤", CalamityUtils.ColorMessage("飘渺不定的幽魂恶兽消失在地牢深处……", new Color(176, 224, 230)), () => true, null, null);
			string name23 = "硫海遗爵";
			...
			BossChecklistPatch.AddBoss(bossChecklist, calamity, name23, difficulty23, npcType15, downed23, summon22, list32, "击败噬魂幽花后的酸雨事件或使用 [i:" + ModContent.ItemType<BloodwormItem>().ToString() + "] 在硫磺海钓鱼召唤", CalamityUtils.ColorMessage("硫海遗爵消失在酸雨风暴中……", new Color(240, 230, 140)), () => true, null, null);
			string name24 = "神明吞噬者";
			...
			BossChecklistPatch.AddBoss(bossChecklist, calamity, name24, difficulty24, npcType16, downed24, summon23, list33, "使用 [i:" + ModContent.ItemType<CosmicWorm>().ToString() + "] 召唤", CalamityUtils.ColorMessage("噬神者撕裂了所有胆敢挑战神明之人，并尽情享用他们的灵魂……", new Color(0, 255, 255)), () => true, delegate (SpriteBatch sb, Rectangle rect, Color color)
			{
			...
			BossChecklistPatch.AddBoss(bossChecklist, calamity, name25, difficulty25, npcType17, downed25, summon24, list34, "在丛林使用 [i:" + ModContent.ItemType<JungleDragonEgg>().ToString() + "] 召唤", CalamityUtils.ColorMessage("犽戎蔑视你的弱小，并不屑于在你的墓碑边多作停留……", new Color(255, 165, 0)), () => true, null, "CalamityMod/NPCs/Yharon/Yharon_Head_Boss");
			string name26 = "星流巨械";
			...
			BossChecklistPatch.AddBoss(bossChecklist, calamity, name26, difficulty26, list35, downed26, summon25, list36, "使用装有金源量子冷却电池的密码破译器召唤", CalamityUtils.ColorMessage("毕竟不是完美造物……真是可惜……", new Color(127, 255, 212)), () => true, delegate (SpriteBatch sb, Rectangle rect, Color color)
			{
			...
			BossChecklistPatch.AddBoss(bossChecklist, calamity, name27, difficulty27, npcType18, downed27, list37, list38, string.Concat(new string[]
			{
				"使用 [i:",
			...
			BossChecklistPatch.AddBoss(bossChecklist, calamity, "成年幻海妖龙", 23.5f, ModContent.NPCType<AdultEidolonWyrmHead>(), () => DownedBossSystem.downedAdultEidolonWyrm, 1326, new List<int>(), "在深渊使用会对自身造成混沌状态减益的物品", CalamityUtils.ColorMessage("……", new Color(127, 255, 212)), () => true, delegate (SpriteBatch sb, Rectangle rect, Color color)
			{
