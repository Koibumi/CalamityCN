...
namespace CalamityCN.Translations.InfernumMode
{
	public class InfernumTooltipTrans : GlobalItem
	{
		...
		public override void ModifyTooltips(Item item, List<TooltipLine> tooltips)
		{
			...
			{
				tooltips.ReplaceText("Summons the Profaned Guardians when used on the cliff in the profaned garden at the far right of the underworld during day", "在地狱最右侧亵渎花园的悬崖上使用召唤亵渎守卫");
				
				ItemHelper.TranslateTooltip(tooltips, "Warning", delegate (TooltipLine tooltip)
				{
			...
			ItemHelper.TranslateTooltip(tooltips, (TooltipLine l) => l.Mod == "InfernumMode" && l.Name == "Developer", delegate (TooltipLine tooltip)
			{
				...
				{
					tooltips.ReplaceText("Hold LMB to teleport to the gate", "按住左键传送到魔法门");
					tooltips.ReplaceText("Hold LMB and", "按住左键和");
					tooltips.ReplaceText("unbound", "未绑定");
					tooltips.ReplaceText("to set the gate to your position", "键，设置你的魔法门传送点");
					tooltips.ReplaceText("to remove the gate", "键，移除魔法门");
				}
				
				if (item.type == ItemID.WormFood)
				{
					tooltips.ReplaceText("Enrages outside of the Corruption", "在腐化外狂暴");
				}
				if (item.type == ItemID.CelestialSigil)
				{
					tooltips.ReplaceText("Summons the Moon Lord immediately\nCreates an arena at the player's position\nNot consumable.", "立刻召唤月亮领主\n会在玩家的位置创建一个战斗场地\n不消耗");
				}
				if (item.type == ModContent.ItemType<EyeofDesolation>())
				{
					tooltips.ReplaceText("Summons the Forgotten Shadow of Calamitas when used during nighttime", "在夜晚使用召唤遗落灾影");
				}
				if (item.type == ModContent.ItemType<ProfanedCore>())
				{
					tooltips.ReplaceText("Summons Providence when used at the altar in the profaned temple at the far right of the underworld", "在地狱最右侧亵渎神庙中圣坛使用召唤亵渎天神");
				}
				if (item.type == ModContent.ItemType<RuneofKos>())
				{
					tooltips.ReplaceText("The Ceaseless Void can only be fought in the Archives", "无尽虚空只能在废弃档案室中战斗");
				}
				if (item.type == ItemID.LihzahrdPowerCell)
				{
					tooltips.ReplaceText("\nCreates a rectangular arena around the altar. If the altar is inside of the temple solid tiles within the arena are broken", "\n在祭坛周围创建一个矩形战斗场地，如果祭坛在神庙内，则场地内神庙方块会被破坏");
				}
				if (item.type == ModContent.ItemType<ProvidenceRelic>())
				{
					tooltips.ReplaceText("The first major hurdle following the defeat of the Moon Lord. Your triumph over her was by no means a small feat.\nPerhaps consider fighting her again during the night for a special challenge?", "这是在你击败月球领主后遇到的第一个阻碍。战胜她绝非易事。\n也许你可以试试在晚上与她来一次特别的战斗？");
					tooltips.ReplaceText("Bruh? What the heck? Are you OK?\nYou were supposed to fight her at night AFTER beating her during the day first!", "老兄？你在干什么？你还好吗？\n你应该先在白天击败过她一次后再在晚上与她战斗的！");
				}
				if (item.type == ModContent.ItemType<DraedonRelic>())
				{
					tooltips.ReplaceText("You have done phenomenally. There is only one challenge left now-\nFace the Witch.", "你做的很好。现在，只剩下一个挑战了-\n面对那位女巫。");
					tooltips.ReplaceText("Spectacular work. You have conquered all of the major obstacles.\nTake pride in this accomplishment, for you are considerably stronger than you were when you began.", "令人赞叹，你已经战胜了所有的困难。\n为这些成就感到骄傲吧，因为你已经比最初的时候更加强大了。");
				}
				...
				{
					tooltips.ReplaceText("The optional foes may at times be the most formidable. So too may they yield the greatest rewards.", "那些可选性的敌人有时可能很难对付。击败他们也可能会拥有很高的收益。");
				}
				if (item.type == ModContent.ItemType<GolemRelic>())
				{
					tooltips.ReplaceText("Simple methodical planning goes a long way. It will be invaluable against future obstacles.", "还有很长的路要走，有条不紊的计划将有助于克服未来的障碍。");
				}
				if (item.type == ModContent.ItemType<KingSlimeRelic>())
				{
					tooltips.ReplaceText("Even seasoned players may struggle somewhat in the face of something new and unfamiliar. Adaptability is key.", "即使是经验再丰富的玩家，面对陌生的挑战也会不熟练。适应力是关键。");
				}
				...
				{
					tooltips.ReplaceText("The first major roadblock. You are better now than before you faced it. Did you have fun learning its patterns?", "流程第一个阻碍，你现在比之前面对它更加熟练了，你享受熟练它攻击模式的过程吗？");
				}
				...
				{
					tooltips.ReplaceText("Opens a portal to the Lost Colosseum", "开启前往失落竞技场的入口");
					tooltips.ReplaceText("Your world does not currently have a Lost Gateway. Kill the Lunatic Cultist again to generate it.", "你的世界目前没有失落之门，杀死拜月教邪教徒以生成它。");
				}
		}
	}
}
