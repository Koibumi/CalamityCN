        ...
        if (IsLanguageActive)
        {
            foreach (TooltipLine line in tooltips)
            {
                ...
                if (item.type == ModContent.ItemType<TrueBiomeBlade>())
                {
					...
					line.Text = line.Text.Replace("Gesture for the Drowned", "溺者之势");
                    line.Text = line.Text.Replace("None", "无");
					...
					line.Text = line.Text.Replace("Shoot out two water balls at your sides. On ground contact the water balls will turn into small waves that ride along the ground", "向两周射出水球。水中击中地面时会化作滑行的小冲击波");
					line.Text = line.Text.Replace("Waves fired while underwater or that touch water will become much larger and faster", "触碰到水的冲击波会变得巨大且飞快");
                }
                ...
                */
                ...
                if (item.type == 1263)
                {
                    line.Text = line.Text.Replace("\nCannot be used while a boss is alive", "在有Boss存活时无法使用");
                }
                #endregion
            }
        }
